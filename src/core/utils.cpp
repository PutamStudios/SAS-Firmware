...
void updateClockTimezone() {
    ...
    timeClient.setTimeOffset(sasConfig.tmz * 3600);

    localTime = timeClient.getEpochTime() + (sasConfig.dst ? 3600 : 0);
...
void showDeviceInfo() {
    ...
    area.addLine("SAS Version: " + String(SAS_VERSION));
    ...
    area.addLine("Brightness: " + String(sasConfig.bright) + "%");
    area.addLine("");
#endif

    area.addLine("[GPIO]");
    area.addLine("GROVE_SDA: " + String(sasConfigPins.i2c_bus.sda));
    area.addLine("GROVE_SCL: " + String(sasConfigPins.i2c_bus.scl));
    area.addLine("SERIAL TX: " + String(sasConfigPins.uart_bus.tx));
    area.addLine("SERIAL RX: " + String(sasConfigPins.uart_bus.rx));
