...
bool setupSdCard() {
#ifndef USE_SD_MMC
    if (sasConfigPins.SDCARD_bus.sck < 0) {
        sdcardMounted = false;
        return false;
    }
#endif
    ...
    if (task) {
        if (!SD.begin((int8_t)sasConfigPins.SDCARD_bus.cs)) result = false;
        // Serial.println("Task not activated");
    }
    // SDCard in the same Bus as TFT, in this case we call the SPI TFT Instance
    else if (sasConfigPins.SDCARD_bus.mosi == (gpio_num_t)TFT_MOSI &&
             sasConfigPins.SDCARD_bus.mosi != GPIO_NUM_NC) {
        ...
        if (!SD.begin(sasConfigPins.SDCARD_bus.cs, tft.getSPIinstance())) {
            result = false;
    ...
    NEXT:
        sdcardSPI.begin(
            (int8_t)sasConfigPins.SDCARD_bus.sck,
            (int8_t)sasConfigPins.SDCARD_bus.miso,
            (int8_t)sasConfigPins.SDCARD_bus.mosi,
            (int8_t)sasConfigPins.SDCARD_bus.cs
        ); // start SPI communications
        delay(10);
        if (!SD.begin((int8_t)sasConfigPins.SDCARD_bus.cs, sdcardSPI)) {
            result = false;
            ...
            if (sasConfigPins.SDCARD_bus.miso != sasConfigPins.CC1101_bus.miso) sdcardSPI.end();
#endif
        }
...
bool copyToFs(FS from, FS to, String path, bool draw) {
    ...
    // tft.drawRect(5,tftHeight-12, (tftWidth-10), 9, sasConfig.priColor);
...
bool pasteFile(FS fs, String path) {
    ...
    while ((bytesRead = sourceFile.read(buff, bufSize)) > 0) {
        if (destFile.write(buff, bytesRead) != bytesRead) {
            ...
            // tft.fillRect(7,tftHeight-10, (tftWidth-14)*prog/tot, 5, sasConfig.priColor);
        }
    }
...
String loopSD(FS &fs, bool filePicker, String allowed_ext, String rootPath) {
    ...
    tft.drawPixel(0, 0, 0);
    tft.fillScreen(sasConfig.bgColor); // TODO: Does only the T-Embed CC1101 need this?
