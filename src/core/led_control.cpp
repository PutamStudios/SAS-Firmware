...
void ledEffectTask(void *pvParameters) {
    ...
    while (1) {
        CRGB baseColor = isPreviewLed ? previewLedColor : sasConfig.ledColor;
        int ledEffect = isPreviewLed ? previewLedEffect : sasConfig.ledEffect;
        int ledEffectSpeed = isPreviewLed ? previewLedEffectSpeed : sasConfig.ledEffectSpeed;
        int ledEffectDirection = isPreviewLed ? previewLedEffectDirection : sasConfig.ledEffectDirection;
...
void beginLed() {
    ...
    */
    ...
    setLedBrightness(sasConfig.ledBright);
}

void blinkLed(int blinkTime) {
    if (!sasConfig.ledBlinkEnabled) return;

    int ledBrightFrom = sasConfig.ledBright;
...
#define SASPurple 9830500 // Custom purple color for SAS
...
void setLedColorConfig() {
    ...
    while (1) {
        ...
        for (const auto &mapping : colorMappings) {
            if (sasConfig.ledColor == mapping.color) { idx = i; }
            colorStorage[i] = mapping.color;

            options.emplace_back(
                mapping.name,
                [=, &mapping]() {
                    // sasConfig.setLedColor(mapping.color);
                    sasConfig.setLedColor(
        ...
        options.push_back(
            {"Custom Color",
             ...
             [](void *pointer, bool shouldRender) {
                 setLedColor(sasConfig.ledColor);
                 return false;
             }}
        );
...
void setCustomColorSettingMenu(int rgb, std::function<uint32_t(uint32_t, int)> colorGenerator) {
    uint32_t originalColor = sasConfig.ledColor;
    ...
    while (i <= 255) {
        if (i % LED_COLOR_STEP == 0 || i == 255) {
            ...
            options.emplace_back(
                String(i),
                [updatedColor]() { sasConfig.setLedColor(updatedColor); },
                selectedIndex == index,
                hoverFunction,
                &colorStorage[index]
            );
...
void setLedEffectConfig() {
    ...
    while (1) {
        options = {
            {"Solid Color",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_SOLID); },
             sasConfig.ledEffect == LED_EFFECT_SOLID,
             [](void *pointer,                                                                         bool shouldRender) {
            ...
            {"Breathe",
             [=]() { sasConfig.setLedEffect(LED_COLOR_BREATHE); },
             sasConfig.ledEffect == LED_COLOR_BREATHE,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_COLOR_BREATHE);
                 return false;
             }                                                                        },
            {"Color Cycle",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_COLOR_CYCLE); },
             sasConfig.ledEffect == LED_EFFECT_COLOR_CYCLE,
             [](void *pointer,                                                                         bool shouldRender) {
...
#if LED_COUNT > 1
            {"Color Wheel",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_COLOR_WHEEL); },
             sasConfig.ledEffect == LED_EFFECT_COLOR_WHEEL,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_EFFECT_COLOR_WHEEL);
                 return false;
             }                                                                        },
            {"Chase",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_CHASE); },
             sasConfig.ledEffect == LED_EFFECT_CHASE,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_EFFECT_CHASE);
                 return false;
             }                                                                        },
            {"Chase Tail",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_CHASE_TAIL); },
             sasConfig.ledEffect == LED_EFFECT_CHASE_TAIL,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_EFFECT_CHASE_TAIL);
                 return false;
             }                                                                        },
            {"Rainbow Chase",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_RAINBOW_CHASE); },
             sasConfig.ledEffect == LED_EFFECT_RAINBOW_CHASE,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_EFFECT_RAINBOW_CHASE);
                 return false;
             }                                                                        },
            {"Rainbow Breathe",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_RAINBOW_BREATHE); },
             sasConfig.ledEffect == LED_EFFECT_RAINBOW_BREATHE,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_EFFECT_RAINBOW_BREATHE);
                 return false;
             }                                                                        },
            {"Fire",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_FIRE); },
             sasConfig.ledEffect == LED_EFFECT_FIRE,
             [](void *pointer,                                                                         bool shouldRender) {
                 setLedEffect(LED_EFFECT_FIRE);
                 return false;
             }                                                                        },
            {"Disco",
             [=]() { sasConfig.setLedEffect(LED_EFFECT_DISCO); },
             sasConfig.ledEffect == LED_EFFECT_DISCO,
             [](void *pointer,                                                                         bool shouldRender) {
            ...
            {"Config - Speed",
             ...
             [](void *pointer,                                                                         bool shouldRender) {
                 previewLedEffect = sasConfig.ledEffect;
                 previewLedEffectSpeed = sasConfig.ledEffectSpeed;
                 previewLedEffectDirection = sasConfig.ledEffectDirection;
                 return false;
             }                                                                        },
        ...
        int selectedOption = loopOptions(options, sasConfig.ledEffect);
        if (selectedOption == -1 || selectedOption == options.size() - 1) {
            ledPreviewMode(false);
            ledSetup();
            return;
...
void setLedEffectSpeedConfig() {
    ...
    while (i < 10) {
        ...
        bool isSelected = (sasConfig.ledEffectSpeed == i + 1);

        options.emplace_back(
            label,
            [i]() { sasConfig.setLedEffectSpeed(i + 1); },
            (sasConfig.ledEffectSpeed == i + 1),
            hoverFunction,
            &speedStorage[i]
        );
        ++i;
    ...
    options.emplace_back(
        "Sync To Encoder",
        []() { sasConfig.setLedEffectSpeed(11); },
        (sasConfig.ledEffectSpeed == 11),
        hoverFunction,
        &speedStorage[10]
    );
#endif
    ...
    int selectedOption = loopOptions(options, sasConfig.ledEffectSpeed - 1);
    if (selectedOption == -1 || selectedOption == options.size() - 1) {
        previewLedEffectSpeed = sasConfig.ledEffectSpeed;
        return;
    }
...
void setLedEffectDirectionConfig() {
    options = {
        {"Clockwise",
         [=]() { sasConfig.setLedEffectDirection(1); },
    ...
    int selectedOption = loopOptions(options, (sasConfig.ledEffectDirection == 1) ? 0 : 1);
    if (selectedOption == -1 || selectedOption == options.size() - 1) {
        previewLedEffectDirection = sasConfig.ledEffectDirection;
        return;
    }
...
void ledSetup() {
    if (sasConfig.ledEffect == LED_EFFECT_SOLID) { ledEffects(false); }

    if (sasConfig.ledEffect > LED_EFFECT_SOLID) {
        ledEffects(true);
    } else setLedColor(sasConfig.ledColor);
}
...
void ledPreviewMode(bool enable) {
    isPreviewLed = enable;
    if (enable) {
        previewLedColor = sasConfig.ledColor;
        previewLedEffect = sasConfig.ledEffect;
        previewLedEffectSpeed = sasConfig.ledEffectSpeed;
        previewLedEffectDirection = sasConfig.ledEffectDirection;
    }
...
void setLedBrightnessConfig() {
    int idx = 0;
    if (sasConfig.ledBright == 0) idx = 0;
    else if (sasConfig.ledBright == 10) idx = 1;
    else if (sasConfig.ledBright == 25) idx = 2;
    else if (sasConfig.ledBright == 50) idx = 3;
    else if (sasConfig.ledBright == 75) idx = 4;
    else if (sasConfig.ledBright == 100) idx = 5;

    options = {
        {"OFF",
         [=]() { sasConfig.setLedBright(0); },
         sasConfig.ledBright == 0,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(0);
             return false;
         }},
        {"10 %",
         [=]() { sasConfig.setLedBright(10); },
         sasConfig.ledBright == 10,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(10);
             return false;
         }},
        {"25 %",
         [=]() { sasConfig.setLedBright(25); },
         sasConfig.ledBright == 25,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(25);
             return false;
         }},
        {"50 %",
         [=]() { sasConfig.setLedBright(50); },
        ...
        {"75 %",
         [=]() { sasConfig.setLedBright(75); },
         sasConfig.ledBright == 75,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(75);
             return false;
         }},
        {"100%",
         [=]() { sasConfig.setLedBright(100); },
         sasConfig.ledBright == 100,
         [](void *pointer, bool shouldRender) {
