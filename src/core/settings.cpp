...
void setBrightness(uint8_t brightval, bool save) {
    if (sasConfig.bright > 100) sasConfig.setBright(100);
    _setBrightness(brightval);
    delay(10);

    currentScreenBrightness = brightval;
    if (save) { sasConfig.setBright(brightval); }
}
...
**********************************************************************/
void getBrightness() {
    if (sasConfig.bright > 100) {
        sasConfig.setBright(100);
        _setBrightness(sasConfig.bright);
        delay(10);
        setBrightness(100);
    }
    ...
    currentScreenBrightness = sasConfig.bright;
}
...
int gsetRotation(bool set) {
    int getRot = sasConfigPins.rotation;
    int result = ROTATION;
    ...
    if (set) {
        sasConfigPins.setRotation(result);
...
void setBrightnessMenu() {
    int idx = 0;
    if (sasConfig.bright == 100) idx = 0;
    else if (sasConfig.bright == 75) idx = 1;
    else if (sasConfig.bright == 50) idx = 2;
    else if (sasConfig.bright == 25) idx = 3;
    else if (sasConfig.bright == 1) idx = 4;

    options = {
        {"100%",
         [=]() { setBrightness((uint8_t)100); },
         sasConfig.bright == 100,
    ...
    setBrightness(sasConfig.bright, false);
}
...
void setDimmerTimeMenu() {
    int idx = 0;
    if (sasConfig.dimmerSet == 10) idx = 0;
    else if (sasConfig.dimmerSet == 20) idx = 1;
    else if (sasConfig.dimmerSet == 30) idx = 2;
    else if (sasConfig.dimmerSet == 60) idx = 3;
    else if (sasConfig.dimmerSet == 0) idx = 4;
    options = {
        {"10s",      [=]() { sasConfig.setDimmer(10); }, sasConfig.dimmerSet == 10},
        {"20s",      [=]() { sasConfig.setDimmer(20); }, sasConfig.dimmerSet == 20},
        {"30s",      [=]() { sasConfig.setDimmer(30); }, sasConfig.dimmerSet == 30},
        {"60s",      [=]() { sasConfig.setDimmer(60); }, sasConfig.dimmerSet == 60},
        {"Disabled", [=]() { sasConfig.setDimmer(0); },  sasConfig.dimmerSet == 0 },
    };
...
void setUIColor() {
    ...
    while (1) {
        ...
        for (const auto &mapping : UI_COLORS) {
            if (sasConfig.priColor == mapping.priColor && sasConfig.secColor == mapping.secColor &&
                sasConfig.bgColor == mapping.bgColor) {
                idx = i;
            }
            ...
            options.emplace_back(
                ...
                [=, &mapping]() {
                    ...
                    sasConfig.setUiColor(mapping.priColor, &secColor, &bgColor);
                },
                idx == i
            );
            ++i;
        ...
        options.push_back(
            {"Custom Color",
             [=]() {
                 uint16_t oldPriColor = sasConfig.priColor;
                 uint16_t oldSecColor = sasConfig.secColor;
                 uint16_t oldBgColor = sasConfig.bgColor;

                 if (setCustomUIColorMenu()) {
                     sasConfig.setUiColor(
                         sasConfig.priColor, &sasConfig.secColor, &sasConfig.bgColor
                     );
                 } else {
                     sasConfig.priColor = oldPriColor;
                     sasConfig.secColor = oldSecColor;
                     sasConfig.bgColor = oldBgColor;
                 }
                 tft.setTextColor(sasConfig.priColor, sasConfig.bgColor);
             },
             idx == UI_COLOR_COUNT}
        );
        ...
        options.push_back(
            {"Invert Color",
             [=]() {
                 sasConfig.setColorInverted(!sasConfig.colorInverted);
                 tft.invertDisplay(sasConfig.colorInverted);
             },
             sasConfig.colorInverted > 0}
        );
...
void setCustomUIColorSettingMenu(
    ...
    uint16_t color = (colorType == 1)   ? sasConfig.priColor
                     : (colorType == 2) ? sasConfig.secColor
                                        : sasConfig.bgColor;
    ...
    static auto hoverFunctionBgColor = [](void *pointer, bool shouldRender) -> bool {
        ...
        sasConfig.bgColor = colorToSet;
        tft.fillScreen(sasConfig.bgColor);
        return false;
    };
        ...
        if (i == 0 || (rgb == 2 && (i + 1) % 2 == 0) || (rgb != 2)) {
            ...
            options.emplace_back(
                ...
                [colorType, updatedColor]() {
                    if (colorType == 1) sasConfig.priColor = updatedColor;
                    else if (colorType == 2) sasConfig.secColor = updatedColor;
                    else sasConfig.bgColor = updatedColor;
                },
                selectedIndex == i,
    ...
    if (selectedOption == -1 || selectedOption == options.size() - 1) {
        if (colorType == 1) {
            sasConfig.priColor = color;
        } else if (colorType == 2) {
            sasConfig.secColor = color;
        } else {
            sasConfig.bgColor = color;
        }
        return;
    }
}
...
void setSoundConfig() {
    options = {
        {"Sound off", [=]() { sasConfig.setSoundEnabled(0); }, sasConfig.soundEnabled == 0},
        {"Sound on",  [=]() { sasConfig.setSoundEnabled(1); }, sasConfig.soundEnabled == 1},
    };
    loopOptions(options, sasConfig.soundEnabled);
}
...
void setSoundVolume() {
    options = {
        {"10%",  [=]() { sasConfig.setSoundVolume(10); },  sasConfig.soundVolume == 10 },
        {"20%",  [=]() { sasConfig.setSoundVolume(20); },  sasConfig.soundVolume == 20 },
        {"30%",  [=]() { sasConfig.setSoundVolume(30); },  sasConfig.soundVolume == 30 },
        {"40%",  [=]() { sasConfig.setSoundVolume(40); },  sasConfig.soundVolume == 40 },
        {"50%",  [=]() { sasConfig.setSoundVolume(50); },  sasConfig.soundVolume == 50 },
        {"60%",  [=]() { sasConfig.setSoundVolume(60); },  sasConfig.soundVolume == 60 },
        {"70%",  [=]() { sasConfig.setSoundVolume(70); },  sasConfig.soundVolume == 70 },
        {"80%",  [=]() { sasConfig.setSoundVolume(80); },  sasConfig.soundVolume == 80 },
        {"90%",  [=]() { sasConfig.setSoundVolume(90); },  sasConfig.soundVolume == 90 },
        {"100%", [=]() { sasConfig.setSoundVolume(100); }, sasConfig.soundVolume == 100},
    };
    loopOptions(options, sasConfig.soundVolume);
}
...
void setLedBlinkConfig() {
    options = {
        {"Led Blink off", [=]() { sasConfig.setLedBlinkEnabled(0); }, sasConfig.ledBlinkEnabled == 0},
        {"Led Blink on",  [=]() { sasConfig.setLedBlinkEnabled(1); }, sasConfig.ledBlinkEnabled == 1},
    };
    loopOptions(options, sasConfig.ledBlinkEnabled);
}
#endif
...
void setWifiStartupConfig() {
    options = {
        {"Disable", [=]() { sasConfig.setWifiAtStartup(0); }, sasConfig.wifiAtStartup == 0},
        {"Enable",  [=]() { sasConfig.setWifiAtStartup(1); }, sasConfig.wifiAtStartup == 1},
    };
    loopOptions(options, sasConfig.wifiAtStartup);
}
...
void addEvilWifiMenu() {
    ...
    if (apName != "\x1B") sasConfig.addEvilWifiName(apName);
}
...
void removeEvilWifiMenu() {
    ...
    for (const auto &wifi_name : sasConfig.evilWifiNames) {
        options.push_back({wifi_name.c_str(), [wifi_name]() { sasConfig.removeEvilWifiName(wifi_name); }});
    }
...
void setEvilEndpointCreds() {
    String userInput = keyboard(sasConfig.evilPortalEndpoints.getCredsEndpoint, 30, "Evil creds endpoint");
    if (userInput != "\x1B") sasConfig.setEvilEndpointCreds(userInput);
}
...
void setEvilEndpointSsid() {
    String userInput = keyboard(sasConfig.evilPortalEndpoints.setSsidEndpoint, 30, "Evil creds endpoint");
    if (userInput != "\x1B") sasConfig.setEvilEndpointSsid(userInput);
}
...
void setEvilAllowGetCreds() {
    options = {
        {"Disallow",
         [=]() { sasConfig.setEvilAllowGetCreds(false); },
         sasConfig.evilPortalEndpoints.allowGetCreds == false},
        {"Allow",
         [=]() { sasConfig.setEvilAllowGetCreds(true); },
         sasConfig.evilPortalEndpoints.allowGetCreds == true },
    };
    loopOptions(options, sasConfig.evilPortalEndpoints.allowGetCreds);
}
...
void setEvilAllowSetSsid() {
    options = {
        {"Disallow",
         [=]() { sasConfig.setEvilAllowSetSsid(false); },
         sasConfig.evilPortalEndpoints.allowSetSsid == false},
        {"Allow",
         [=]() { sasConfig.setEvilAllowSetSsid(true); },
         sasConfig.evilPortalEndpoints.allowSetSsid == true },
    };
    loopOptions(options, sasConfig.evilPortalEndpoints.allowSetSsid);
}
...
void setEvilAllowEndpointDisplay() {
    options = {
        {"Disallow",
         [=]() { sasConfig.setEvilAllowEndpointDisplay(false); },
         sasConfig.evilPortalEndpoints.showEndpoints == false},
        {"Allow",
         [=]() { sasConfig.setEvilAllowEndpointDisplay(true); },
         sasConfig.evilPortalEndpoints.showEndpoints == true },
    };
    loopOptions(options, sasConfig.evilPortalEndpoints.showEndpoints);
}
...
void setEvilPasswordMode() {
    options = {
        {"Save 'password'",
         [=]() { sasConfig.setEvilPasswordMode(FULL_PASSWORD); },
         sasConfig.evilPortalPasswordMode == FULL_PASSWORD  },
        {"Save 'p******d'",
         [=]() { sasConfig.setEvilPasswordMode(FIRST_LAST_CHAR); },
         sasConfig.evilPortalPasswordMode == FIRST_LAST_CHAR},
        {"Save '*hidden*'",
         [=]() { sasConfig.setEvilPasswordMode(HIDE_PASSWORD); },
         sasConfig.evilPortalPasswordMode == HIDE_PASSWORD  },
        {"Save length",
         [=]() { sasConfig.setEvilPasswordMode(SAVE_LENGTH); },
         sasConfig.evilPortalPasswordMode == SAVE_LENGTH    },
    };
    loopOptions(options, sasConfig.evilPortalPasswordMode);
}
...
void setRFModuleMenu() {
    ...
    if (sasConfigPins.rfModule == M5_RF_MODULE) idx = 0;
    else if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        idx = 1;
#if defined(ARDUINO_M5STICK_C_PLUS) || defined(ARDUINO_M5STICK_C_PLUS2)
        if (sasConfigPins.CC1101_bus.mosi == GPIO_NUM_26) idx = 2;
#endif
    }
    ...
    if (result == CC1101_SPI_MODULE || pins_setup > 0) {
        ...
        if (initRfModule()) {
            sasConfigPins.setRfModule(CC1101_SPI_MODULE);
            deinitRfModule();
        ...
        if (pins_setup == 1)
            qrcode_display("https://github.com/pr3y/SAS/blob/main/media/connections/cc1101_stick.jpg");
        if (pins_setup == 2)
            qrcode_display(
                "https://github.com/pr3y/SAS/blob/main/media/connections/cc1101_stick_SDCard.jpg"
            );
        while (!check(AnyKeyPress)) vTaskDelay(50 / portTICK_PERIOD_MS);
    }
    // fallback to "M5 RF433T/R" on errors
    sasConfigPins.setRfModule(M5_RF_MODULE);
}
...
void setRFFreqMenu() {
    float result = 433.92;
    String freq_str = num_keyboard(String(sasConfigPins.rfFreq), 10, "Default frequency:");
    ...
    if (freq_str.length() > 1) {
        ...
        if (result >= 280 && result <= 928) { // TODO: check valid freq according to current module?
            sasConfigPins.setRfFreq(result);
            return;
        }
    }
    // else
    displayError("Invalid frequency");
    sasConfigPins.setRfFreq(433.92); // reset to default
    delay(1000);
}
...
void setRFIDModuleMenu() {
    options = {
        {"M5 RFID2",
         [=]() { sasConfigPins.setRfidModule(M5_RFID2_MODULE); },
         sasConfigPins.rfidModule == M5_RFID2_MODULE     },
#ifdef M5STICK
        {"PN532 I2C G33",
         [=]() { sasConfigPins.setRfidModule(PN532_I2C_MODULE); },
         sasConfigPins.rfidModule == PN532_I2C_MODULE    },
        {"PN532 I2C G36",
         [=]() { sasConfigPins.setRfidModule(PN532_I2C_SPI_MODULE); },
         sasConfigPins.rfidModule == PN532_I2C_SPI_MODULE},
#else
        {"PN532 on I2C",
         [=]() { sasConfigPins.setRfidModule(PN532_I2C_MODULE); },
         sasConfigPins.rfidModule == PN532_I2C_MODULE},
#endif
        {"PN532 on SPI",
         [=]() { sasConfigPins.setRfidModule(PN532_SPI_MODULE); },
         sasConfigPins.rfidModule == PN532_SPI_MODULE    },
        {"RC522 on SPI",
         [=]() { sasConfigPins.setRfidModule(RC522_SPI_MODULE); },
         sasConfigPins.rfidModule == RC522_SPI_MODULE    },
    };
    loopOptions(options, sasConfigPins.rfidModule);
}
...
void addMifareKeyMenu() {
    ...
    if (key != "\x1B") sasConfig.addMifareKey(key);
}
...
void setClock() {
    ...
    options = {
        {"Via NTP Set Timezone",                                                 [&]() { sasConfig.setAutomaticTimeUpdateViaNTP(true); } },
        {"Set Time Manually",                                                    [&]() { sasConfig.setAutomaticTimeUpdateViaNTP(false); }},
        {("Daylight Savings " + String(sasConfig.dst ? "On" : "Off")).c_str(),
         [&]() {
             sasConfig.setDST(!sasConfig.dst);
             updateClockTimezone();
             returnToMenu = true;
         }                                                                                                                                 },
        {(sasConfig.clock24hr ? "24-Hour Format" : "12-Hour Format"),          [&]() {
             sasConfig.setClock24Hr(!sasConfig.clock24hr);
             returnToMenu = true;
         }                                                          }
    };
    ...
    if (sasConfig.automaticTimeUpdateViaNTP) {
        ...
        for (const auto &mapping : timezoneMappings) {
            if (sasConfig.tmz == mapping.offset) { idx = i; }

            options.emplace_back(
                mapping.name, [=, &mapping]() { sasConfig.setTmz(mapping.offset); }, idx == i
            );
            ++i;
        }
        ...
        for (const auto &offset : timezoneOffsets) {
            if (sasConfig.tmz == offset) idx = i;

            options.emplace_back(
                ("UTC" + String(offset >= 0 ? "+" : "") + String(offset)).c_str(),
                [=]() { sasConfig.setTmz(offset); },
                sasConfig.tmz == offset
            );
            ++i;
        }
...
int gsetIrTxPin(bool set) {
    int result = sasConfigPins.irTx;

    if (result > 50) sasConfigPins.setIrTxPin(TXLED);
    if (set) {
        options.clear();
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == sasConfigPins.irTx && idx == 100) idx = j;
            j++;
            ...
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
                ...
                options.push_back(
                    {pin.first,
                     [=]() { sasConfigPins.setIrTxPin(pin.second); },
                     pin.second == sasConfigPins.irTx}
                );
        }
        ...
        Serial.println("Saved pin: " + String(sasConfigPins.irTx));
    }

    returnToMenu = true;
    return sasConfigPins.irTx;
}
...
void setIrTxRepeats() {
    ...
    options = {
        ...
        {"Custom",           [&]() {
             // up to 99 repeats
             String rpt =
                 num_keyboard(String(sasConfigPins.irTxRepeats), 2, "Nbr of Repeats (+ 1 initial)");
    ...
    sasConfigPins.setIrTxRepeats(chRpts);
}
/*********************************************************************
...
int gsetIrRxPin(bool set) {
    int result = sasConfigPins.irRx;

    if (result > 45) sasConfigPins.setIrRxPin(GROVE_SCL);
    if (set) {
        options.clear();
        std::vector<std::pair<const char *, int>> pins;
        pins = IR_RX_PINS;
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == sasConfigPins.irRx && idx < 0) idx = j;
            j++;
#ifdef ALLOW_ALL_GPIO_FOR_IR_RF
            int i = pin.second;
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
                ...
                options.push_back(
                    {pin.first,
                     [=]() { sasConfigPins.setIrRxPin(pin.second); },
                     pin.second == sasConfigPins.irRx}
                );
        }
...
int gsetRfTxPin(bool set) {
    int result = sasConfigPins.rfTx;

    if (result > 45) sasConfigPins.setRfTxPin(GROVE_SDA);
    if (set) {
        options.clear();
        std::vector<std::pair<const char *, int>> pins;
        pins = RF_TX_PINS;
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == sasConfigPins.rfTx && idx < 0) idx = j;
            j++;
#ifdef ALLOW_ALL_GPIO_FOR_IR_RF
            int i = pin.second;
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
                ...
                options.push_back(
                    {pin.first,
                     [=]() { sasConfigPins.setRfTxPin(pin.second); },
                     pin.second == sasConfigPins.rfTx}
                );
        }
...
int gsetRfRxPin(bool set) {
    int result = sasConfigPins.rfRx;

    if (result > 36) sasConfigPins.setRfRxPin(GROVE_SCL);
    if (set) {
        options.clear();
        std::vector<std::pair<const char *, int>> pins;
        pins = RF_RX_PINS;
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == sasConfigPins.rfRx && idx < 0) idx = j;
            j++;
#ifdef ALLOW_ALL_GPIO_FOR_IR_RF
            int i = pin.second;
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
                ...
                options.push_back(
                    {pin.first,
                     [=]() { sasConfigPins.setRfRxPin(pin.second); },
                     pin.second == sasConfigPins.rfRx}
                );
        }
...
void setStartupApp() {
    int idx = 0;
    if (sasConfig.startupApp == "") idx = 0;

    options = {
        {"None", [=]() { sasConfig.setStartupApp(""); }, sasConfig.startupApp == ""}
    };

    int index = 0;
    for (String appName : startupApp.getAppNames()) {
        index++;
        if (sasConfig.startupApp == appName) idx = index;

        options.push_back({appName.c_str(), [=]() {
                               sasConfig.setStartupApp(appName);
#if !defined(LITE_VERSION) && !defined(DISABLE_INTERPRETER)
...
void setGpsBaudrateMenu() {
    options = {
        {"9600 bps",   [=]() { sasConfigPins.setGpsBaudrate(9600); },  sasConfigPins.gpsBaudrate == 9600 },
        {"19200 bps",  [=]() { sasConfigPins.setGpsBaudrate(19200); }, sasConfigPins.gpsBaudrate == 19200},
        {"38400 bps",  [=]() { sasConfigPins.setGpsBaudrate(38400); }, sasConfigPins.gpsBaudrate == 38400},
        {"57600 bps",  [=]() { sasConfigPins.setGpsBaudrate(57600); }, sasConfigPins.gpsBaudrate == 57600},
        {"115200 bps",
         [=]() { sasConfigPins.setGpsBaudrate(115200); },
         sasConfigPins.gpsBaudrate == 115200                                                               },
    };

    loopOptions(options, sasConfigPins.gpsBaudrate);
}
...
void setWifiApSsidMenu() {
    const bool isDefault = sasConfig.wifiAp.ssid == "SASNet";

    options = {
        {"Default (SASNet)",
         [=]() { sasConfig.setWifiApCreds("SASNet", sasConfig.wifiAp.pwd); },
         isDefault                                                                            },
        {"Custom",
         [=]() {
             String newSsid = keyboard(sasConfig.wifiAp.ssid, 32, "WiFi AP SSID:");
             if (newSsid != "\x1B") {
                 if (!newSsid.isEmpty()) sasConfig.setWifiApCreds(newSsid, sasConfig.wifiAp.pwd);
...
void setWifiApPasswordMenu() {
    const bool isDefault = sasConfig.wifiAp.pwd == "sasnet";

    options = {
        {"Default (sasnet)",
         [=]() { sasConfig.setWifiApCreds(sasConfig.wifiAp.ssid, "sasnet"); },
         isDefault                                                                             },
        {"Custom",
         [=]() {
             String newPassword = keyboard(sasConfig.wifiAp.pwd, 32, "WiFi AP Password:", true);
             if (newPassword != "\x1B") {
                 if (!newPassword.isEmpty()) sasConfig.setWifiApCreds(sasConfig.wifiAp.ssid, newPassword);
...
void setBadUSBBLEKeyboardLayoutMenu() {
    uint8_t opt = sasConfig.badUSBBLEKeyboardLayout;
    ...
    if (opt != sasConfig.badUSBBLEKeyboardLayout) { sasConfig.setBadUSBBLEKeyboardLayout(opt); }
}
...
void setBadUSBBLEKeyDelayMenu() {
    String delayStr = num_keyboard(String(sasConfig.badUSBBLEKeyDelay), 3, "Key Delay (ms):");
    if (delayStr != "\x1B") {
        uint16_t delayVal = static_cast<uint16_t>(delayStr.toInt());
        if (delayVal <= 500) {
            sasConfig.setBadUSBBLEKeyDelay(delayVal);
        } else if (delayVal != 0) {
...
void setBadUSBBLEShowOutputMenu() {
    options.clear();
    options = {
        {"Enable",  [&]() { sasConfig.setBadUSBBLEShowOutput(true); } },
        {"Disable", [&]() { sasConfig.setBadUSBBLEShowOutput(false); }},
    };
    addOptionToMainMenu();

    loopOptions(options, sasConfig.badUSBBLEShowOutput ? 0 : 1);
}
...
void setMacAddressMenu() {
    String currentMAC = sasConfig.wifiMAC;
    if (currentMAC == "") currentMAC = WiFi.macAddress();

    options.clear();
    options = {
        {"Default MAC (" + WiFi.macAddress() + ")",
         [&]() { sasConfig.setWifiMAC(""); },
         sasConfig.wifiMAC == ""},
        {"Set Custom MAC",
         [&]() {
             String newMAC = keyboard(sasConfig.wifiMAC, 17, "XX:YY:ZZ:AA:BB:CC");
             if (newMAC == "\x1B") return;
             if (newMAC.length() == 17) {
                 sasConfig.setWifiMAC(newMAC);
             } else {
        ...
        {"Random MAC", [&]() {
             ...
             sasConfig.setWifiMAC(String(buf));
         }}
    };
...
**********************************************************************/
void setSPIPinsMenu(SASConfigPins::SPIPins &value) {
    uint8_t opt = 0;
    bool changed = false;
    SASConfigPins::SPIPins points = value;
...
RELOAD:
    ...
    if (opt == 0) return;
        ...
        if (changed) {
            value = points;
            sasConfigPins.setSpiPins(value);
        }
    } else {
        options = {};
...
**  Main Menu to manually set SPI Pins
**********************************************************************/
void setUARTPinsMenu(SASConfigPins::UARTPins &value) {
    uint8_t opt = 0;
    bool changed = false;
    SASConfigPins::UARTPins points = value;
...
RELOAD:
    ...
    if (opt == 0) return;
        ...
        if (changed) {
            value = points;
            sasConfigPins.setUARTPins(value);
        }
    } else {
        options = {};
        gpio_num_t sel = GPIO_NUM_NC;
...
**  Main Menu to manually set SPI Pins
**********************************************************************/
void setI2CPinsMenu(SASConfigPins::I2CPins &value) {
    uint8_t opt = 0;
    bool changed = false;
    SASConfigPins::I2CPins points = value;
...
RELOAD:
    ...
    if (opt == 0) return;
        ...
        if (changed) {
            value = points;
            sasConfigPins.setI2CPins(value);
        }
    } else {
        options = {};
        gpio_num_t sel = GPIO_NUM_NC;
...
void setTheme() {
    ...
    options = {
        ...
        {"Default",
         [&]() {
             sasConfig.removeTheme();
             sasConfig.themePath = "";
             sasConfig.theme.fs = 0;
             sasConfig.secColor = DEFAULT_SECCOLOR;
             sasConfig.bgColor = TFT_BLACK;
             sasConfig.setUiColor(DEFAULT_PRICOLOR);
#ifdef HAS_RGB_LED
             sasConfig.ledBright = 50;
             sasConfig.ledColor = 0x960064;
             sasConfig.ledEffect = 0;
             sasConfig.ledEffectSpeed = 5;
             sasConfig.ledEffectDirection = 1;
             ledSetup();
#endif
             sasConfig.saveFile();
             fs = nullptr;
    ...
    if (sasConfig.openThemeFile(fs, filepath, true)) {
        sasConfig.themePath = filepath;
        if (fs == &LittleFS) sasConfig.theme.fs = 1;
        else if (fs == &SD) sasConfig.theme.fs = 2;
        else sasConfig.theme.fs = 0;
...
bool appStoreInstalled() {
    ...
    return fs->exists("/SASJS/Tools/App Store.js");
}
...
void installAppStoreJS() {
    ...
    if (!fs->exists("/SASJS")) {
        if (!fs->mkdir("/SASJS")) {
            displayWarning("Failed to create /SASJS directory", true);
            return;
        }
    }

    if (!fs->exists("/SASJS/Tools")) {
        if (!fs->mkdir("/SASJS/Tools")) {
            displayWarning("Failed to create /SASJS/Tools directory", true);
            return;
        }
    }
    ...
    File file = fs->open("/SASJS/Tools/App Store.js", FILE_WRITE);
    if (!file) {
