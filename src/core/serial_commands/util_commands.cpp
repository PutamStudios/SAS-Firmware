...
uint32_t i2cCallback(cmd *c) {
    ...
    Wire.begin(sasConfigPins.i2c_bus.sda, sasConfigPins.i2c_bus.scl);
    byte error, address;
    int nDevices;
...
uint32_t infoCallback(cmd *c) {
    serialDevice->print("SAS v");
    serialDevice->println(SAS_VERSION);
...
uint32_t helpCallback(cmd *c) {
    serialDevice->print("SAS v");
    serialDevice->print(SAS_VERSION);
    ...
    serialDevice->println("  deauth target <ap_mac> [client_mac] [channel] - Background targeted deauth");
    serialDevice->println("  deauth flood - Continuously deauth flood all WiFi channels");
    serialDevice->println("  deauth stop  - Terminate background deauth task safely");
    serialDevice->println("\nWebUI Commands:");
    ...
    serialDevice->println("\nSAS Custom Commands:");
    serialDevice->println("  sas                     - Show premium SAS custom diagnostics and metrics.");
    serialDevice->println("  speed [80/160/240]      - Dynamically configure or query CPU frequency (MHz).");
...
uint32_t speedCallback(cmd *c) {
    Command cmd(c);
    Argument valArg = cmd.getArgument("value");
    String val = valArg.getValue();
    val.trim();

    if (val == "") {
        serialDevice->printf("[System] Current CPU Frequency: %d MHz\n", getCpuFrequencyMhz());
        return true;
    }

    int target_speed = val.toInt();
    if (target_speed == 80 || target_speed == 160 || target_speed == 240) {
        setCpuFrequencyMhz(target_speed);
        serialDevice->printf("[System] CPU Frequency successfully configured to: %d MHz\n", getCpuFrequencyMhz());
        return true;
    } else {
        serialDevice->println("[System] Invalid frequency selection. Choose 80, 160, or 240 MHz.");
        return false;
    }
}

uint32_t sasCallback(cmd *c) {
    serialDevice->println("\n=================================");
    serialDevice->println("███████  █████  ███████");
    serialDevice->println("██      ██   ██ ██");
    serialDevice->println("███████ ███████ ███████");
    serialDevice->println("     ██ ██   ██      ██");
    serialDevice->println("███████ ██   ██ ███████  v" SAS_VERSION);
    serialDevice->println("=================================");
    serialDevice->println("SAS CUSTOM FIRMWARE DIAGNOSTICS:");
    ...
    int min = (sec / 60) % 60;
    sec = sec % 60;
    serialDevice->printf("  Uptime: %02d:%02d:%02d\n", hr, min, sec);
    serialDevice->printf("  CPU Speed: %d MHz\n", getCpuFrequencyMhz());
    
    // Heap Memory
    serialDevice->printf("  Free Heap: %d bytes (Total: %d)\n", ESP.getFreeHeap(), ESP.getHeapSize());
    
    // PSRAM Memory
    if (psramFound()) {
        serialDevice->printf("  Free PSRAM: %d bytes (Total: %d)\n", ESP.getFreePsram(), ESP.getPsramSize());
    } else {
        serialDevice->println("  PSRAM: Not available");
    }
    ...
    // Network interface
    if (wifiConnected) {
        serialDevice->println("  WiFi Connection: CONNECTED");
        serialDevice->println("  WiFi IP: " + wifiIP);
    } else {
        serialDevice->println("  WiFi Connection: DISCONNECTED");
    }
...
void createUtilCommands(SimpleCLI *cli) {
    ...
    cli->addCommand("sas,sas-fw,sas_info", sasCallback);
    ...
    Command sysSpeed = cli->addCommand("speed,sysspeed,clock_hz", speedCallback);
    sysSpeed.addPosArg("value", "");
}
