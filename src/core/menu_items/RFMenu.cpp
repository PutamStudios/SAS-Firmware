...
void RFMenu::optionsMenu() {
    ...
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) txt += " (CC1101)"; // Indicates if CC1101 is connected
    else txt += " Tx: " + String(sasConfigPins.rfTx) + " Rx: " + String(sasConfigPins.rfRx);
...
void RFMenu::drawIcon(float scale) {
    ...
    // Body
    tft.fillCircle(iconCenterX, iconCenterY - radius, radius, sasConfig.priColor);
    tft.fillTriangle(
        iconCenterX,
        iconCenterY,
