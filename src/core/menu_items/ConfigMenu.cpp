...
void ConfigMenu::ledMenu() {
    while (true) {
        std::vector<Option> localOptions = {
            ...
            {String("LED Blink: ") + (sasConfig.ledBlinkEnabled ? "ON" : "OFF"),
             [this]() {
                 // Toggle LED blink setting
                 sasConfig.ledBlinkEnabled = !sasConfig.ledBlinkEnabled;
                 sasConfig.saveFile();
             }                                                                            },
            {"Back",                                                               []() {}},
        };
...
void ConfigMenu::audioMenu() {
    while (true) {
        std::vector<Option> localOptions = {
            ...
            {String("Sound: ") + (sasConfig.soundEnabled ? "ON" : "OFF"),
                                                             [this]() {
                 // Toggle sound setting
                 sasConfig.soundEnabled = !sasConfig.soundEnabled;
                 sasConfig.saveFile();
             }                                                                                                                                            },
...
void ConfigMenu::systemMenu() {
    while (true) {
        std::vector<Option> localOptions = {
            {String("InstaBoot: ") + (sasConfig.instantBoot ? "ON" : "OFF"),
             [this]() {
                 // Toggle InstaBoot setting
                 sasConfig.instantBoot = !sasConfig.instantBoot;
                 sasConfig.saveFile();
             }                                                                                                           },
            {String("WiFi Startup: ") + (sasConfig.wifiAtStartup ? "ON" : "OFF"),
             [this]() {
                 // Toggle WiFi at startup setting
                 sasConfig.wifiAtStartup = !sasConfig.wifiAtStartup;
                 sasConfig.saveFile();
             }                                                                                                           },
            {"Startup App",                                                         [this]() { setStartupApp(); }        },
            {"Hide/Show Apps",                                                      [this]() { mainMenu.hideAppsMenu(); }},
            {"Clock",                                                               [this]() { setClock(); }             },
            {String("Keyboard Language: ") + sasConfig.keyboardLang,             [this]() { setKeyboardLanguage(); }  },
        ...
        std::vector<Option> localOptions = {
            ...
            {"Factory Reset",
                 ...
                 if (choice == 1) {
                     ...
                     sasConfigPins.factoryReset();
                     sasConfig.factoryReset(); // Restarts ESP
                 }
...
void ConfigMenu::devMenu() {
    while (true) {
        std::vector<Option> localOptions = {
            ...
            {"CC1101 Pins",     [this]() { setSPIPinsMenu(sasConfigPins.CC1101_bus); }},
            {"NRF24  Pins",     [this]() { setSPIPinsMenu(sasConfigPins.NRF24_bus); } },
#if !defined(LITE_VERSION)
            {"LoRa Pins",       [this]() { setSPIPinsMenu(sasConfigPins.LoRa_bus); }  },
            {"W5500 Pins",      [this]() { setSPIPinsMenu(sasConfigPins.W5500_bus); } },
#endif
            {"SDCard Pins",     [this]() { setSPIPinsMenu(sasConfigPins.SDCARD_bus); }},
            {"I2C Pins",        [this]() { setI2CPinsMenu(sasConfigPins.i2c_bus); }   },
            {"UART Pins",       [this]() { setUARTPinsMenu(sasConfigPins.uart_bus); } },
            {"GPS Pins",        [this]() { setUARTPinsMenu(sasConfigPins.gps_bus); }  },
            {"Serial USB",      [this]() { switchToUSBSerial(); }                       },
            {"Serial UART",     [this]() { switchToUARTSerial(); }                      },
            {"Disable DevMode", [this]() { sasConfig.setDevMode(false); }             },
            {"Back",            []() {}                                                 },
        };
...
void ConfigMenu::switchToUARTSerial() {
    ...
    if (sasConfigPins.SDCARD_bus.checkConflict(sasConfigPins.uart_bus.rx) ||
        sasConfigPins.SDCARD_bus.checkConflict(sasConfigPins.uart_bus.tx)) {
        sdcardSPI.end();
    }

    // Check and resolve CC1101/NRF24 pin conflicts
    if (sasConfigPins.CC1101_bus.checkConflict(sasConfigPins.uart_bus.rx) ||
        sasConfigPins.CC1101_bus.checkConflict(sasConfigPins.uart_bus.tx) ||
        sasConfigPins.NRF24_bus.checkConflict(sasConfigPins.uart_bus.rx) ||
        sasConfigPins.NRF24_bus.checkConflict(sasConfigPins.uart_bus.tx)) {
        CC_NRF_SPI.end();
    }

    // Configure UART pins and switch serial output
    pinMode(sasConfigPins.uart_bus.rx, INPUT);
    pinMode(sasConfigPins.uart_bus.tx, OUTPUT);
    Serial1.begin(115200, SERIAL_8N1, sasConfigPins.uart_bus.rx, sasConfigPins.uart_bus.tx);
