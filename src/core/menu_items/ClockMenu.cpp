...
void ClockMenu::drawIcon(float scale) {
    ...
    // Case
    tft.drawArc(
        iconCenterX, iconCenterY, 1.1 * radius, radius, 0, 360, sasConfig.priColor, sasConfig.bgColor
    );

    // Pivot center
    tft.fillCircle(iconCenterX, iconCenterY, radius / 10, sasConfig.priColor);
    ...
    tft.drawLine(
        iconCenterX, iconCenterY, iconCenterX + pointerSize, iconCenterY - pointerSize, sasConfig.priColor
    );
}
