...
void GpsMenu::optionsMenu() {
    ...
    String txt = "GPS (" + String(sasConfigPins.gpsBaudrate) + " bps)";
...
void GpsMenu::configMenu() {
    options = {
        ...
        {"GPS Pins", [=]() { setUARTPinsMenu(sasConfigPins.gps_bus); }},
        {"Back",     [this]() { optionsMenu(); }                        },
    };
...
void GpsMenu::drawIcon(float scale) {
    ...
    tft.fillCircle(iconCenterX, iconCenterY - radius / 2, radius, sasConfig.priColor);
    tft.fillTriangle(
    ...
    tft.fillCircle(iconCenterX, iconCenterY - radius / 2, radius / 2, sasConfig.bgColor);

    tft.drawEllipse(iconCenterX, iconCenterY + 1.5 * radius, 1.5 * radius, radius / 2, sasConfig.priColor);
}
