...
void OthersMenu::drawIcon(float scale) {
    ...
    // Center circle
    tft.fillCircle(iconCenterX, iconCenterY, radius, sasConfig.priColor);

    // Concentric arcs - dynamically scaled for different screen sizes
    tft.drawArc(
        iconCenterX, iconCenterY, 2.5 * radius, 2 * radius, 0, 340, sasConfig.priColor, sasConfig.bgColor
    );

    tft.drawArc(
        iconCenterX, iconCenterY, 3.5 * radius, 3 * radius, 20, 360, sasConfig.priColor, sasConfig.bgColor
    );

    tft.drawArc(
        iconCenterX, iconCenterY, 4.5 * radius, 4 * radius, 0, 200, sasConfig.priColor, sasConfig.bgColor
    );
