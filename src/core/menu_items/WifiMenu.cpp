...
//  Use SASC2 to interact with the reverse shell server
//  SASC2: https://github.com/9dl/SAS-C2
//  To use SASC2:
//  1. Start Reverse Shell Mode in SAS
//  2. Start SASC2 and wait.
//  3. Visit 192.168.4.1 in your browser to access the web interface for shell executing.

// 32bit: https://github.com/9dl/SAS-C2/releases/download/v1.0/SASC2_windows_386.exe
// 64bit: https://github.com/9dl/SAS-C2/releases/download/v1.0/SASC2_windows_amd64.exe
...
void WifiMenu::optionsMenu() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        options = {
            ...
            {"Start WiFi AP", [=]() {
                 ...
                 displayInfo("pwd: " + sasConfig.wifiAp.pwd, true);
             }},
        };
    }
    ...
    options.push_back({"SASgotchi", sasgotchi_start});
...
void WifiMenu::drawIcon(float scale) {
    ...
    tft.fillCircle(iconCenterX, iconCenterY + deltaY, radius, sasConfig.priColor);
    tft.drawArc(
        iconCenterX,
        iconCenterY + deltaY,
        deltaY + radius,
