...
void IRMenu::optionsMenu() {
    ...
    String txt = "Infrared";
    txt += " Tx: " + String(sasConfigPins.irTx) + " Rx: " + String(sasConfigPins.irRx) +
           " Rpts: " + String(sasConfigPins.irTxRepeats);
...
void IRMenu::drawIcon(float scale) {
    ...
    tft.fillRect(
        iconCenterX - iconSize / 2, iconCenterY - iconSize / 2, iconSize / 6, iconSize, sasConfig.priColor
    );
    tft.fillRect(
    ...
    tft.drawCircle(iconCenterX - iconSize / 6, iconCenterY, radius, sasConfig.priColor);
