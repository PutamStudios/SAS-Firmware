...
void SASConfigPins::fromJson(JsonObject obj) {
    int count = 0;
...
void SASConfigPins::toJson(JsonObject obj) const {
...
void SASConfigPins::loadFile(JsonDocument &jsonDoc, bool checkFS) {
    FS *fs;
    if (checkFS) {
...
void SASConfigPins::fromFile(bool checkFS) {
...
void SASConfigPins::factoryReset() {
    FS *fs = &LittleFS;
...
void SASConfigPins::validateConfig() {
...
#if !defined(LITE_VERSION)
void SASConfigPins::setLoRaPins(SPIPins value) {
    LoRa_bus = value;
    validateSpiPins(LoRa_bus);
    saveFile();
}
void SASConfigPins::setW5500Pins(SPIPins value) {
    LoRa_bus = value;
    validateSpiPins(W5500_bus);
    saveFile();
}
#endif
void SASConfigPins::setCC1101Pins(SPIPins value) {
    CC1101_bus = value;
    validateSpiPins(CC1101_bus);
    saveFile();
}

void SASConfigPins::setNrf24Pins(SPIPins value) {
    NRF24_bus = value;
    validateSpiPins(NRF24_bus);
    saveFile();
}

void SASConfigPins::setSDCardPins(SPIPins value) {
    SDCARD_bus = value;
    validateSpiPins(SDCARD_bus);
    saveFile();
}

void SASConfigPins::setSpiPins(SPIPins value) {
    validateSpiPins(value);
    saveFile();
}

void SASConfigPins::setI2CPins(I2CPins value) {
    validateI2CPins(value);
    saveFile();
}

void SASConfigPins::setUARTPins(UARTPins value) {
    validateUARTPins(value);
    saveFile();
}
void SASConfigPins::validateSpiPins(SPIPins value) {
...
void SASConfigPins::validateI2CPins(I2CPins value) {
...
void SASConfigPins::validateUARTPins(UARTPins value) {
    if (value.rx < 0 || value.rx > GPIO_PIN_COUNT) value.rx = GPIO_NUM_NC;
    if (value.tx < 0 || value.tx > GPIO_PIN_COUNT) value.tx = GPIO_NUM_NC;
}

void SASConfigPins::setRotation(int value) {
    rotation = value;
    validateRotationValue();
    saveFile();
}

void SASConfigPins::validateRotationValue() {
    if (rotation < 0 || rotation > 3) rotation = 1;
}

void SASConfigPins::setBleName(String value) {
    bleName = value;
    saveFile();
}

void SASConfigPins::setIrTxPin(int value) {
    irTx = value;
    saveFile();
}

void SASConfigPins::setIrTxRepeats(uint8_t value) {
    irTxRepeats = value;
    saveFile();
}

void SASConfigPins::setIrRxPin(int value) {
    irRx = value;
    saveFile();
}

void SASConfigPins::setRfTxPin(int value) {
    rfTx = value;
    saveFile();
}

void SASConfigPins::setRfRxPin(int value) {
    rfRx = value;
    saveFile();
}

void SASConfigPins::setRfModule(RFModules value) {
    rfModule = value;
    validateRfModuleValue();
    saveFile();
}

void SASConfigPins::validateRfModuleValue() {
    if (rfModule != M5_RF_MODULE && rfModule != CC1101_SPI_MODULE) { rfModule = M5_RF_MODULE; }
}

void SASConfigPins::setRfFreq(float value, int fxdFreq) {
    rfFreq = value;
    if (fxdFreq > 1) rfFxdFreq = fxdFreq;
    saveFile();
}

void SASConfigPins::setRfFxdFreq(float value) {
    rfFxdFreq = value;
    saveFile();
}

void SASConfigPins::setRfScanRange(int value, int fxdFreq) {
    rfScanRange = value;
    rfFxdFreq = fxdFreq;
    validateRfScanRangeValue();
    saveFile();
...
void SASConfigPins::validateRfScanRangeValue() {
    if (rfScanRange < 0 || rfScanRange > 3) rfScanRange = 3;
}

void SASConfigPins::setRfidModule(RFIDModules value) {
    rfidModule = value;
    validateRfidModuleValue();
    saveFile();
}

void SASConfigPins::validateRfidModuleValue() {
...
void SASConfigPins::setiButtonPin(int value) {
    if (value < GPIO_NUM_MAX) {
        iButton = value;
        saveFile();
    } else log_e("iButton: Gpio pin not set, incompatible with this device\n");
}
void SASConfigPins::setGpsBaudrate(int value) {
    gpsBaudrate = value;
    validateGpsBaudrateValue();
    saveFile();
}

void SASConfigPins::validateGpsBaudrateValue() {
