...
void SASTheme::removeTheme(void) {
    themeInfo t;
    theme = t;
}
FS *SASTheme::themeFS(void) {
    if (theme.fs == 1) return &LittleFS;
    else if (theme.fs == 2) return &SD;
    return &LittleFS; // always get back to safety
}
bool SASTheme::openThemeFile(FS *fs, String filepath, bool overwriteConfigSettings) {
    ...
    if (overwriteConfigSettings) {
        uint16_t _priColor = sasConfig.priColor;
        uint16_t _secColor = sasConfig.secColor;
        uint16_t _bgColor = sasConfig.bgColor;
...
#ifdef HAS_RGB_LED
        if (!_th["ledBright"].isNull()) { sasConfig.ledBright = _th["ledBright"].as<int>(); }
        if (!_th["ledColor"].isNull()) { sasConfig.ledColor = strtoul(_th["ledColor"], nullptr, 16); }
        if (!_th["ledEffect"].isNull()) { sasConfig.ledEffect = _th["ledEffect"].as<int>(); }
        if (!_th["ledEffectSpeed"].isNull()) { sasConfig.ledEffectSpeed = _th["ledEffectSpeed"].as<int>(); }
        if (!_th["ledEffectDirection"].isNull()) {
            sasConfig.ledEffectDirection = _th["ledEffectDirection"].as<int>();
        }
        ledSetup();
#endif
    }
...
bool SASTheme::validateImgFile(FS *fs, String filepath) {
...
void SASTheme::_setUiColor(uint16_t primary, uint16_t *secondary, uint16_t *background) {
    priColor = primary;
