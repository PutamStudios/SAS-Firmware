...
void applyConfiguredMAC() {
    if (sasConfig.wifiMAC.length() == 17 && validateMACFormat(sasConfig.wifiMAC)) {
        uint8_t newMAC[6];
        sscanf(
            sasConfig.wifiMAC.c_str(),
        ...
        if (esp_wifi_set_mac(WIFI_IF_STA, newMAC) == ESP_OK) {
            Serial.println("[WiFi] Custom MAC applied: " + sasConfig.wifiMAC);
        } else {
...
void wifiMACMenu() {
    ...
    if (sasConfig.wifiMAC != "" && validateMACFormat(sasConfig.wifiMAC)) {
        currentMAC = sasConfig.wifiMAC + " (Custom)";
    } else {
