...
bool _wifiConnect(const String &ssid, int encryption) {
    String password = sasConfig.getWifiPassword(ssid);
    ...
    if (connected) {
        ...
        sasConfig.addWifiCredential(ssid, password);
...
bool _setupAP() {
    ...
    WiFi.softAP(sasConfig.wifiAp.ssid, sasConfig.wifiAp.pwd, 6, 0, 4, false);
...
void wifiConnectTask(void *pvParameters) {
    ...
    for (int i = 0; i < nets; i++) {
        ssid = WiFi.SSID(i);
        pwd = sasConfig.getWifiPassword(ssid);
        if (pwd == "") continue;
