...
bool checkUserWebAuth(AsyncWebServerRequest *request, bool onFailureReturnLoginPage = false) {
    if (request->hasHeader("Cookie")) {
        ...
        int idx = c.indexOf("SASSESSION=");
        if (idx != -1) {
            int start = idx + 13;
            int end = c.indexOf(';', start);
            if (end == -1) end = c.length();
            String token = c.substring(start, end);
            if (sasConfig.isValidWebUISession(token)) { return true; }
        }
    }
...
void drawWebUiScreen(bool mode_ap) {
    ...
    if (mode_ap) {
        setTftDisplay(0, 0, sasConfig.bgColor, FM);
        tft.drawCentreString("SASNet/sasnet", tftWidth / 2, 7, 1);
    }
    setTftDisplay(0, 0, ALCOLOR, FM);
    tft.drawCentreString("SAS WebUI", tftWidth / 2, 27, 1);
    String txt;
    if (!mode_ap) txt = WiFi.localIP().toString();
    else txt = WiFi.softAPIP().toString();
    tft.setTextColor(sasConfig.priColor);

    tft.drawCentreString("http://sas.local", tftWidth / 2, 45, 1);
    setTftDisplay(7, 67);
    ...
    tft.println("Usr: " + String(sasConfig.webUI.user));
    tft.setCursor(7, tft.getCursorY());
    tft.println("Pwd: " + String(sasConfig.webUI.pwd));
    tft.setCursor(7, tft.getCursorY());
...
void serveWebUIFile(
    ...
    FS *fs = NULL;
    if (setupSdCard()) {
        if (SD.exists("/SASWebUI/" + filename)) fs = &SD;
    } else if (LittleFS.exists("/SASWebUI/" + filename)) {
        fs = &LittleFS;
    }
    if (fs) {
        response = request->beginResponse(*fs, "/SASWebUI/" + filename, contentType);
    } else {
        if (filename == "theme.css") {
            String css = ":root{--color:" + color565ToWebHex(sasConfig.priColor) +
                         ";--sec-color:" + color565ToWebHex(sasConfig.secColor) +
                         ";--background:" + color565ToWebHex(sasConfig.bgColor) + ";}";
...
void configureWebServer() {
    ...
    server->on("/login", HTTP_POST, [](AsyncWebServerRequest *request) {
        if (request->hasParam("username", true) && request->hasParam("password", true)) {
            ...
            if (username == sasConfig.webUI.user && password == sasConfig.webUI.pwd) {
                String token = generateToken();
                AsyncWebServerResponse *response = request->beginResponse(302);
                response->addHeader("Location", "/");
                response->addHeader("Set-Cookie", "SASSESSION=" + token + "; Path=/; HttpOnly");
                request->send(response);
                sasConfig.addWebUISession(token);
                return;
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
    ...
    server->on("/logout", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasHeader("Cookie")) {
            ...
            if (idx != -1) {
                ...
                if (end == -1) end = c.length();
                String token = c.substring(start, end);
                sasConfig.removeWebUISession(token);
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
        response->addHeader("Location", "/?loggedout");
        response->addHeader("Set-Cookie", "SASSESSION=0; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
        request->send(response);
    });
    ...
    server->on("/wifi", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            if (request->hasArg("usr") && request->hasArg("pwd")) {
                ...
                sasConfig.setWebUICreds(usr, pwd);
                request->send(
