...
void displayTextLine(String txt, bool waitKeyPress) {
    // todo: add newlines to txt if too long
    displayRedStripe(txt, getComplementaryColor2(sasConfig.priColor), sasConfig.priColor);
#ifndef HAS_SCREEN
...
int loopOptions(
    ...
    while (1) {
        ...
        if (menuType == MENU_TYPE_MAIN) {
            checkReboot();
            if (devModeCounter >= 5 && !sasConfig.devMode) {
                sasConfig.setDevMode(true);
        ...
        if (redraw) {
            ...
            if (!renderedByLambda) {
                ...
                else if (menuType == MENU_TYPE_MAIN) drawMainVerticalMenu(index, options);
                else
                    coord = drawOptions(
                        index,
                        options,
            ...
            while (PrevPress && menuType != MENU_TYPE_MAIN) {
                if (millis() - _tmp > 200)
                    tft.drawArc(
                        ...
                        getColorVariation(sasConfig.priColor),
                        sasConfig.bgColor
                    );
                vTaskDelay(10 / portTICK_RATE_MS);
            }
            tft.drawArc(
                tftWidth / 2, tftHeight / 2, 25, 15, 0, 360, sasConfig.bgColor, sasConfig.bgColor
            );
            LongPress = false;
#endif
        ...
        if (check(NextPress) || check(DownPress)) {
            ...
            if ((index + 1) > options.size()) {
                if (!sasConfig.devMode) devModeCounter++;
                index = 0;
            }
            redraw = true;
        }
...
void progressHandler(int progress, size_t total, String message) {
    ...
    if (barWidth < 3) {
        tft.fillRect(6, 27, tftWidth - 12, tftHeight - 33, sasConfig.bgColor);
        tft.drawRect(18, tftHeight - 47, tftWidth - 36, 17, sasConfig.priColor);
        displayRedStripe(message, TFT_WHITE, sasConfig.priColor);
    }
    tft.fillRect(20, tftHeight - 45, barWidth, 13, sasConfig.priColor);
}
...
Opt_Coord drawOptions(
    ...
    bool isPortrait = !(sasConfigPins.rotation & 0b01);
    int fontSize = isPortrait ? FP : FM;
    int32_t boxX = isPortrait ? 6 : tftWidth * 0.10;
    int32_t boxW = isPortrait ? tftWidth - 12 : tftWidth * 0.8;

    // Uncomment to update the statusBar (causes flickering)
    // drawStatusBar();

    int32_t optionsTopY = tftHeight / 2 - menuSize * (fontSize * 8 + 4) / 2 - 5;
    tft.drawPixel(0, 0, sasConfig.bgColor);
    if (firstRender) {
        tft.fillRoundRect(
            boxX, optionsTopY, boxW, (fontSize * 8 + 4) * menuSize + 10, 5, bgcolor
        );
        tft.drawRoundRect(
            boxX,
            tftHeight / 2 - menuSize * (fontSize * 8 + 4) / 2 - 5,
            boxW,
            (fontSize * 8 + 4) * menuSize + 10,
            5,
            fgcolor
        );
    }
    ...
    tft.setCursor(boxX + 5, tftHeight / 2 - menuSize * (fontSize * 8 + 4) / 2);
    ...
    for (i = 0; i < menuSize; i++) {
        if (i >= init) {
            ...
            if (i == index) {
                text += ">";
                coord.x = boxX + 5 + fontSize * LW;
                coord.y = tft.getCursorY() + 4;
                coord.size = (boxW - 10) / (LW * fontSize) - 1;
                coord.fgcolor = fgcolor;
                coord.bgcolor = bgcolor;
            } else text += " ";
            text += String(options[i].label) + "              ";
            tft.setCursor(boxX + 5, tft.getCursorY() + 4);
            tft.println(text.substring(0, (boxW - 10) / (LW * fontSize) - 1));
            cont++;
        }
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    tft.setTextSize(FP);
    tft.drawPixel(0, 0, 0);
    tft.fillRect(6, 30, tftWidth - 12, 8 * FP, sasConfig.bgColor);
    ...
    tft.setTextColor(sasConfig.secColor);
    tft.fillRect(6, middle_up, tftWidth - 12, 8 * FM, sasConfig.bgColor);
    ...
    tft.setTextColor(sasConfig.priColor);
    tft.fillRect(6, middle - FG * LH / 2 - 1, tftWidth - 12, FG * LH + 5, sasConfig.bgColor);
    ...
    tft.setTextSize(FM);
    tft.setTextColor(sasConfig.secColor);
    tft.fillRect(6, middle_down, tftWidth - 12, 8 * FM, sasConfig.bgColor);
    tft.drawCentreString(thirdOption, tftWidth / 2, middle_down, SMOOTH_FONT);

    tft.fillRect(tftWidth - 5, 0, 5, tftHeight, sasConfig.bgColor);
    tft.fillRect(tftWidth - 5, index * tftHeight / menuSize, 5, tftHeight / menuSize, sasConfig.priColor);

#if defined(HAS_TOUCH)
    tft.drawCentreString("\\/", tftWidth / 2, middle_down + (FM * LH + 6), 1);
    tft.setTextColor(getColorVariation(sasConfig.priColor), sasConfig.bgColor);
    tft.drawString("[ x ]", 7, 7, 1);
    TouchFooter();
#endif
}

void drawMainVerticalMenu(int index, std::vector<Option> &options) {
    uint16_t pri = sasConfig.priColor;
    uint16_t sec = sasConfig.secColor;
    uint16_t bg = sasConfig.bgColor;

    // Clear background
    tft.fillRect(0, 30, tftWidth, tftHeight - 30, bg);

    // Draw status bar
    drawStatusBar();

    // Responsive row height calculation based on available screen space
    int usableHeight = tftHeight - 35;
    int rowHeight = usableHeight / 3;
    int numOptions = options.size();

    // Draw visible items (Previous, Selected, Next)
    int visibleIndices[3];
    visibleIndices[0] = (index - 1 + numOptions) % numOptions; // Top/Prev item
    visibleIndices[1] = index;                                 // Selected item
    visibleIndices[2] = (index + 1) % numOptions;              // Bottom/Next item

    for (int r = 0; r < 3; r++) {
        int optIndex = visibleIndices[r];
        int rowCenterY = 32 + r * rowHeight + rowHeight / 2;

        // Highlight selected row with a beautiful rounded rectangle
        if (r == 1) {
            tft.drawRoundRect(6, rowCenterY - rowHeight / 2 + 2, tftWidth - 22, rowHeight - 4, 6, pri);
        }

        MenuItemInterface *menuItem = static_cast<MenuItemInterface *>(options[optIndex].hoverPointer);
        if (menuItem) {
            // Temporarily override menuItem coordinates to place the icon on the left
            menuItem->iconCenterX = 24;
            menuItem->iconCenterY = rowCenterY;
            menuItem->iconAreaW = 24;
            menuItem->iconAreaH = 24;
            menuItem->iconAreaX = 24 - 12;
            menuItem->iconAreaY = rowCenterY - 12;

            // Draw miniature icon
            menuItem->drawIcon(0.28);

            // Restore base class default coordinates
            menuItem->resetCoordinates();
        }

        // Draw menu item text
        tft.setTextSize(FM);
        tft.setTextColor(r == 1 ? pri : sec, bg);
        tft.setCursor(48, rowCenterY - 8);
        tft.print(options[optIndex].label);
    }

    // Draw scroll bar on the right
    int scrollTrackY = 35;
    int scrollTrackH = tftHeight - 45;
    tft.drawFastVLine(tftWidth - 8, scrollTrackY, scrollTrackH, sec);

    int thumbHeight = scrollTrackH / numOptions;
    if (thumbHeight < 8) thumbHeight = 8;
    int maxThumbTravel = scrollTrackH - thumbHeight;
    int thumbY = scrollTrackY;
    if (numOptions > 1) {
        thumbY += (index * maxThumbTravel) / (numOptions - 1);
    }
    tft.fillRoundRect(tftWidth - 10, thumbY, 4, thumbHeight, 2, pri);
}
...
void drawStatusBar() {
    ...
    if (sasConfig.theme.border) {
        tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, sasConfig.priColor);
        tft.drawLine(5, 25, tftWidth - 6, 25, sasConfig.priColor);
    }

    if (clock_set) {
        int clock_fontsize = 1; // Font size of the clock / SAS + SAS_VERSION
        setTftDisplay(12, 12, sasConfig.priColor, clock_fontsize, sasConfig.bgColor);
        tft.fillRect(12, 12, 100, clock_fontsize * LH, sasConfig.bgColor);
#if defined(HAS_RTC)
        ...
        setTftDisplay(12, 12, sasConfig.priColor, 1, sasConfig.bgColor);
        tft.print("SAS " + String(SAS_VERSION));
    }
}
...
void drawMainBorder(bool clear) {
    ...
    // if(wifiConnected) {tft.print(timeStr);} else {tft.print("SAS 1.0b");}
...
void printCenterFootnote(String text) {
    tft.fillRect(10, tftHeight - BORDER_PAD_X - FP * LH, tftWidth - 20, FP * LH, sasConfig.bgColor);
    tft.setTextSize(FP);
...
void drawBatteryStatus(uint8_t bat) {
    ...
    uint16_t color = sasConfig.priColor;
    uint16_t barcolor = sasConfig.priColor;
    if (bat < 16) barcolor = color = TFT_RED;
    else if (bat < 34) barcolor = color = TFT_YELLOW;
    if (charging) color = TFT_GREEN;

    tft.drawRoundRect(tftWidth - 43, 6, 36, 19, 2, charging ? color : sasConfig.bgColor); // (bolder border)
    ...
    tft.drawLine(tftWidth - 30, 9, tftWidth - 30, 9 + 13, sasConfig.bgColor);
    tft.drawLine(tftWidth - 20, 9, tftWidth - 20, 9 + 13, sasConfig.bgColor);
}
/***************************************************************************************
...
void drawWireguardStatus(int x, int y) {
    tft.fillRect(x, y, 20, 17, sasConfig.bgColor);
    if (isConnectedWireguard) {
        tft.drawRoundRect(11 + x, 0 + y, 8, 12, 5, TFT_GREEN);
        tft.fillRoundRect(10 + x, 8 + y, 10, 8, 0, TFT_GREEN);
    } else {
        tft.drawRoundRect(1 + x, 0 + y, 8, 12, 5, sasConfig.priColor);
        tft.fillRoundRect(0 + x, 8 + y, 10, 8, 0, sasConfig.bgColor);
        tft.fillRoundRect(6 + x, 8 + y, 10, 10, 0, sasConfig.priColor);
    }
}
    ...
    while (i < arraySize) {
        if (i >= start) {
            ...
            if (fileList[i].folder == true)
                tft.setTextColor(getColorVariation(sasConfig.priColor), sasConfig.bgColor);
            else if (fileList[i].operation == true) tft.setTextColor(ALCOLOR, sasConfig.bgColor);
            else { tft.setTextColor(sasConfig.priColor, sasConfig.bgColor); }
            ...
            if (index == i) {
                ...
                coord.size = nchars;
                coord.fgcolor =
                    fileList[i].folder ? getColorVariation(sasConfig.priColor) : sasConfig.priColor;
                coord.bgcolor = sasConfig.bgColor;
            } else txt = " ";
...
void drawWifiSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, sasConfig.bgColor);
    tft.fillCircle(9 + x, 14 + y, 1, sasConfig.priColor);
    tft.drawArc(9 + x, 14 + y, 4, 6, 130, 230, sasConfig.priColor, sasConfig.bgColor);
    tft.drawArc(9 + x, 14 + y, 10, 12, 130, 230, sasConfig.priColor, sasConfig.bgColor);
}

void drawWebUISmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, sasConfig.bgColor);

    tft.drawCircle(8 + x, 8 + y, 7, sasConfig.priColor);

    tft.drawLine(3 + x, 4 + y, 14 + x, 4 + y, sasConfig.priColor);
    tft.drawLine(2 + x, 8 + y, 15 + x, 8 + y, sasConfig.priColor);
    tft.drawLine(3 + x, 12 + y, 14 + x, 12 + y, sasConfig.priColor);
}

void drawBLESmall(int x, int y) {
    tft.fillRect(x, 2 + y, 17, 13, sasConfig.bgColor);
    tft.drawWideLine(8 + x, 8 + y, 4 + x, 5 + y, 2, sasConfig.priColor, sasConfig.bgColor);
    tft.drawWideLine(8 + x, 8 + y, 4 + x, 13 + y, 2, sasConfig.priColor, sasConfig.bgColor);
    tft.drawTriangle(8 + x, 8 + y, 8 + x, 2 + y, 13 + x, 5 + y, sasConfig.priColor);
    tft.drawTriangle(8 + x, 8 + y, 8 + x, 14 + y, 13 + x, 11 + y, sasConfig.priColor);
}

void drawBLE_beacon(int x, int y, uint16_t color) {
    tft.fillRect(x, y, 40, 80, sasConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 2 + x, 26 + y, 5, color, sasConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 2 + x, 53 + y, 5, color, sasConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 20 + x, 68 + y, 5, color, sasConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 20 + x, 12 + y, 5, color, sasConfig.bgColor);
    tft.drawWideLine(20 + x, 12 + y, 20 + x, 68 + y, 5, color, sasConfig.bgColor);
...
void drawGPS(int x, int y) {
    tft.fillRect(x, y, 80, 80, sasConfig.bgColor);
    tft.drawEllipse(40 + x, 70 + y, 15, 8, sasConfig.priColor);
    tft.drawArc(40 + x, 25 + y, 23, 7, 0, 340, sasConfig.priColor, sasConfig.bgColor);
    tft.fillTriangle(40 + x, 70 + y, 20 + x, 64 + y, 60 + x, 64 + y, sasConfig.priColor);
}

void drawGpsSmall(int x, int y) {
    tft.fillRect(x, y, 17, 17, sasConfig.bgColor);
    tft.drawEllipse(9 + x, 14 + y, 4, 3, sasConfig.priColor);
    tft.drawArc(9 + x, 6 + y, 5, 2, 0, 340, sasConfig.priColor, sasConfig.bgColor);
    tft.fillTriangle(9 + x, 15 + y, 5 + x, 9 + y, 13 + x, 9 + y, sasConfig.priColor);
}

void drawCreditCard(int x, int y) {
    tft.fillRect(x, y, 70, 50, sasConfig.bgColor);
    tft.fillRoundRect(x + 5, y + 5, 60, 40, 5, sasConfig.priColor);
    tft.fillRect(x + 5, y + 15, 60, 10, getColorVariation(sasConfig.priColor, 3, -1));
    tft.fillRect(x + 10, y + 30, 12, 10, getColorVariation(sasConfig.priColor, 3, 1));
    tft.drawRect(x + 10, y + 30, 12, 10, getColorVariation(sasConfig.priColor, 5, -1));
    tft.drawRect(x + 10 + 4, y + 30, 4, 10, getColorVariation(sasConfig.priColor, 5, -1));
    tft.drawRect(x + 10, y + 33, 5, 4, getColorVariation(sasConfig.priColor, 5, -1));
    tft.drawRect(x + 17, y + 33, 5, 4, getColorVariation(sasConfig.priColor, 5, -1));
    tft.fillRect(x + 30, y + 35, 30, 5, getColorVariation(sasConfig.priColor, 5, 1));
}

void drawMfkey32Icon(int x, int y) {
    tft.drawRect(x + 2, y + 15, 24, 40, sasConfig.priColor);
    tft.drawRect(x + 5, y + 18, 18, 12, sasConfig.priColor);
    tft.drawRect(x + 5, y + 34, 18, 18, sasConfig.priColor);
    tft.drawLine(x + 5, y + 40, x + 22, y + 40, sasConfig.priColor);
    tft.drawLine(x + 5, y + 46, x + 22, y + 46, sasConfig.priColor);
    tft.drawLine(x + 11, y + 34, x + 11, y + 51, sasConfig.priColor);
    tft.drawLine(x + 17, y + 34, x + 17, y + 51, sasConfig.priColor);
    tft.drawRect(x + 30, y + 10, 25, 35, sasConfig.priColor);
    int startX = x + 32;
    int startY = y + 12;
    int endX = x + 52;
    int endY = y + 32;
    ...
    while (startX <= endX && startY <= endY && turns < 3) {
        for (int i = startX; i <= endX; i++) { tft.drawPixel(i, startY, sasConfig.priColor); }
        startY += step;
        for (int i = startY; i <= endY; i++) { tft.drawPixel(endX, i, sasConfig.priColor); }
        endX -= step;
        for (int i = endX; i >= startX; i--) { tft.drawPixel(i, endY, sasConfig.priColor); }
        endY -= step;
        for (int i = endY; i >= startY; i--) { tft.drawPixel(startX, i, sasConfig.priColor); }
        startX += step;
        turns++;
    }
    tft.fillRect(x + 40, y + 36, 6, 6, getColorVariation(sasConfig.priColor, 3, 1));
}

void drawMfkey64Icon(int x, int y) {
    drawMfkey32Icon(x, y);
    tft.fillRoundRect(x + 40, y + 6, 24, 14, 4, sasConfig.bgColor);
    tft.drawRoundRect(x + 40, y + 6, 24, 14, 4, getColorVariation(sasConfig.priColor, 3, -1));
    tft.drawCircle(x + 48, y + 12, 4, getColorVariation(sasConfig.priColor, 3, -1));
}
...
int PNGDraw(PNGDRAW *pDraw) {
    ...
    uint8_t r = ((uint16_t)sasConfig.bgColor & 0xF800) >> 8;
    uint8_t g = ((uint16_t)sasConfig.bgColor & 0x07E0) >> 3;
    uint8_t b = ((uint16_t)sasConfig.bgColor & 0x001F) << 3;
