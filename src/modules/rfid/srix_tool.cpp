...
void SRIXTool::setup() {
    ...
    int sda_pin = sasConfigPins.i2c_bus.sda;
    int scl_pin = sasConfigPins.i2c_bus.scl;
...
void SRIXTool::save_file() {
    ...
    if (!(*fs).exists("/SASRFID/SRIX")) (*fs).mkdir("/SASRFID/SRIX");

    // Check if the file already exists and add number
    String filepath = "/SASRFID/SRIX/" + filename;
    ...
    // Write header
    file.println("Filetype: SAS SRIX Dump");
...
void SRIXTool::load_file() {
    ...
    if (!(*fs).exists("/SASRFID/SRIX")) {
        displayError("No dumps found!");
        delay(1500);
        displayError("Folder /SASRFID/SRIX missing");
        delay(2000);
        set_state(IDLE_MODE);
        return;
    }
    ...
    for (const String &fname : fileList) {
        options.emplace_back(fname, [this, fname, fs]() { load_file_data(fs, "/SASRFID/SRIX/" + fname); });
    }
...
void SRIXTool::load_file_data(FS *fs, String filepath) {
    ...
    int lastSlash = filename.lastIndexOf('/');
    if (lastSlash >= 0) {
        filename = filename.substring(lastSlash + 1); // Remove "/SASRFID/SRIX/"
    }
