...
bool Chameleon::readFileLF() {
    ...
    if (!(*fs).exists("/SASRFID")) (*fs).mkdir("/SASRFID");
    filepath = loopSD(*fs, true, "RFIDLF", "/SASRFID");
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists("/SASRFID")) (*fs).mkdir("/SASRFID");
    if ((*fs).exists("/SASRFID/" + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/SASRFID/" + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/SASRFID/" + filename + ".rfidlf", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: SAS RFID 125kHz File");
...
bool Chameleon::readFileHF() {
    ...
    if (!getFsStorage(fs)) return false;
    if (!(*fs).exists("/SASRFID")) (*fs).mkdir("/SASRFID");
    filepath = loopSD(*fs, true, "RFID|NFC", "/SASRFID");
    file = fs->open(filepath, FILE_READ);
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists("/SASRFID")) (*fs).mkdir("/SASRFID");
    if ((*fs).exists("/SASRFID/" + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/SASRFID/" + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/SASRFID/" + filename + ".rfid", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: SAS RFID File");
    file.println("Version 1");
...
void Chameleon::saveScanResult() {
    ...
    if (!(*fs).exists("/SASRFID")) (*fs).mkdir("/SASRFID");
    if (!(*fs).exists("/SASRFID/Scans")) (*fs).mkdir("/SASRFID/Scans");
    if ((*fs).exists("/SASRFID/Scans/" + filename + ".rfidscan")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/SASRFID/Scans/" + filename + String(i) + ".rfidscan")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/SASRFID/Scans/" + filename + ".rfidscan", FILE_WRITE);

    if (!file) { return; }

    file.println("Filetype: SAS RFID Scan Result");
