...
void PN532KillerTools::setup() {
    // Reset Pin states
    if (sasConfigPins.SDCARD_bus.checkConflict(RXD_PIN) ||
        sasConfigPins.SDCARD_bus.checkConflict(TXD_PIN)) {
        sdcardSPI.end();
    }
    if (sasConfigPins.CC1101_bus.checkConflict(RXD_PIN) ||
        sasConfigPins.CC1101_bus.checkConflict(TXD_PIN) ||
        sasConfigPins.NRF24_bus.checkConflict(RXD_PIN) ||
        sasConfigPins.NRF24_bus.checkConflict(TXD_PIN)) {
        CC_NRF_SPI.end();
    }
...
void PN532KillerTools::playDeviceDetectedSound() {
    ...
    if (SD.exists("/device_detected.wav")) {
        ...
        if (sasConfig.theme.boot_sound) {
            playAudioFile(
                sasConfig.themeFS(), sasConfig.getThemeItemImg(sasConfig.theme.paths.boot_sound)
            );
