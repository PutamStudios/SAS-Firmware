...
void TagOMatic::set_rfid_module() {
    switch (sasConfigPins.rfidModule) {
...
void TagOMatic::read_card() {
    ...
    if (_rfid->read() != RFIDInterface::SUCCESS) {
        if (sasConfigPins.rfidModule != M5_RFID2_MODULE) { // Read felica if module is PN532
...
int TagOMatic::load_file_headless(String filename) {
    ...
    String filepath = "/SASRFID/" + filename;
    ...
    // Parse file .rfid (standard RFID format for SAS)

    // Filetype: SAS RFID File
    // Version: 1
    // Device type: <tipo>
    // UID: <uid>
    // SAK: <sak>
