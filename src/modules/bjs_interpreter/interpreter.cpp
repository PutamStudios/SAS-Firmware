...
void interpreterHandler(void *pvParameters) {
    ...
    JS_SetPropertyStr(ctx, global, "SAS_VERSION", JS_NewString(ctx, SAS_VERSION));
    JS_SetPropertyStr(ctx, global, "SAS_PRICOLOR", JS_NewInt32(ctx, sasConfig.priColor));
    JS_SetPropertyStr(ctx, global, "SAS_SECCOLOR", JS_NewInt32(ctx, sasConfig.secColor));
    JS_SetPropertyStr(ctx, global, "SAS_BGCOLOR", JS_NewInt32(ctx, sasConfig.bgColor));
...
String getScriptsFolder(FS *&fs) {
    String folder;
    String possibleFolders[] = {"/scripts", "/SASScripts", "/SASJS"};
        ...
        if (isDir) {
            ...
            opt.push_back({entry_title.c_str(), [=]() {
                               if (saveStartupScript) {
                                   sasConfig.startupAppJSInterpreterFile = fullPath;
                                   sasConfig.saveFile();
                               } else {
