...
JSValue native_getDeviceName(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    const char *deviceName = sasConfig.wifiAp.ssid != NULL ? sasConfig.wifiAp.ssid.c_str() : "SAS";
...
JSValue native_getSASVersion(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    return JS_NewString(ctx, SAS_VERSION);
}
