...
JSValue native_subghzRead(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    ...
    if (argc > 0 && JS_IsNumber(ctx, argv[0])) {
        ...
        r = RCSwitch_Read(sasConfigPins.rfFreq, t, false, true); // headless mode for JS
    } else {
        r = RCSwitch_Read(sasConfigPins.rfFreq, 10, false, true);
    }
...
JSValue native_subghzReadRaw(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    ...
    if (argc > 0 && JS_IsNumber(ctx, argv[0])) {
        int t;
        JS_ToInt32(ctx, &t, argv[0]);
        r = RCSwitch_Read(sasConfigPins.rfFreq, t, true, true); // raw + headless for JS
    } else {
        r = RCSwitch_Read(sasConfigPins.rfFreq, 10, true, true);
    }
    return JS_NewString(ctx, r.c_str());
}
...
JSValue native_subghzSetFrequency(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    ...
    if (argc > 0 && JS_IsNumber(ctx, argv[0])) {
        ...
        sasConfigPins.rfFreq = v; // float global var
    }
    return JS_UNDEFINED;
}
...
JSValue native_subghzTxSetup(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    ...
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        _txPin = sasConfigPins.CC1101_bus.io0;
    } else {
        _txPin = sasConfigPins.rfTx;
    }
