...
void responder() {
    ...
    tft.fillScreen(sasConfig.bgColor);
    // M5Cardputer.Display.setTextColor(sasConfig.priColor, sasConfig.bgColor);
    if (!wifiConnected) wifiConnectMenu();

    netbiosname_str = keyboard("SAS", 20);
    netbiosName = stringTochar(netbiosname_str);
    netbiosdomain_str = keyboard("SASGROUP", 20);
    netbiosDomain = stringTochar(netbiosdomain_str);
    dnsdomain_str = keyboard("SAS.Local", 20);
