...
static bool ensureKarmaApInterface(uint8_t channel) {
    ...
    if (!isApModeActive()) {
        ...
        if (!WiFi.softAP("SASKarma", "", channel, 1, 4, false)) {
...
void saveHandshakeToFile(const HandshakeCapture &hs) {
    ...
    String filename = "/SASPCAP/handshakes/HS_" + String(macStr) + "_" + hs.ssid + ".pcap";
    filename.replace(" ", "_");
    filename.replace("*", "");
...
void updateKarmaDisplay() {
    ...
    if (currentTime - last_time > 1000) {
        ...
        tft.fillRect(10, 45, tftWidth - 20, tftHeight - 70, sasConfig.bgColor);
        tft.setTextSize(1);
        tft.setTextColor(sasConfig.priColor, sasConfig.bgColor);
            ...
            std::vector<Option> options = {
                ...
                {"SSID Database",
                 ...
                 [&]() {
                              ...
                              int y = 60;
                              tft.setTextSize(1);
                              tft.fillRect(10, 40, tftWidth - 20, 100, sasConfig.bgColor);
                              size_t total = SSIDDatabase::getCount();
                              tft.setCursor(10, y);
                              y += 15;
                ...
                {"View Captures",
                 ...
                 [&]() {
                              FS *fs;
                              if (getFsStorage(fs) && fs->exists("/SASPCAP/handshakes")) {
                                  loopSD(*fs, false, "PCAP", "/SASPCAP/handshakes");
                              } else {
