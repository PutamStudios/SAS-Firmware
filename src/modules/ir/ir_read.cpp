...
void IrRead::setup() {
    ...
    int count = 0;
    for (auto pin : pins) {
        if (pin.second == sasConfigPins.irRx) count++;
    }
    if (count == 0) gsetIrRxPin(true); // Open dialog to choose irRx pin

    setup_ir_pin(sasConfigPins.irRx, INPUT_PULLUP);
    if (headless) return;
    // else
...
bool IrRead::write_file(String filename, FS *fs) {
    ...
    while ((*fs).exists("/SASIR/" + filename + ".ir")) {
        int ch = 1;
        int i = 1;
        ...
        switch (ch) {
            case 1:
                filename += "_";
                while ((*fs).exists("/SASIR/" + filename + String(i) + ".ir")) i++;
                filename += String(i);
                break;
            case 2: (*fs).remove("/SASIR/" + filename + ".ir"); break;
            case 3:
    ...
    if ((*fs).exists("/SASIR/" + filename + ".ir")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/SASIR/" + filename + String(i) + ".ir")) i++;
        filename += String(i);
    ...
    */
    ...
    File file = (*fs).open("/SASIR/" + filename + ".ir", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: SAS IR File");
