...
bool txIrFile(FS *fs, String filepath, bool hideDefaultUI) {
    ...
    setup_ir_pin(sasConfigPins.irTx, OUTPUT);
    // digitalWrite(sasConfigPins.irTx, LED_ON);
    ...
    resetCodesArray();
    digitalWrite(sasConfigPins.irTx, LED_OFF);
    return true;
}
...
void sendNECCommand(String address, String command, bool hideDefaultUI) {
    IRsend irsend(sasConfigPins.irTx); // Set the GPIO to be used to sending the message.
    irsend.begin();
    ...
    if (sasConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.sendNEC(data, 32); }
    }

    Serial.println(
        "Sent NEC Command" + (sasConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
...
void sendNECextCommand(String address, String command, bool hideDefaultUI) {
    ...
    Serial.println(
        "Sent NECext Command" + (sasConfigPins.irTxRepeats > 0
                                     ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                     : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
void sendRC5Command(String address, String command, bool hideDefaultUI) {
    IRsend irsend(sasConfigPins.irTx, true); // Set the GPIO to be used to sending the message.
    irsend.begin();
    if (!hideDefaultUI) { displayTextLine("Sending.."); }
    ...
    if (sasConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.sendRC5(data, 13); }
    }
    Serial.println(
        "Sent RC5 Command" + (sasConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
void sendRC6Command(String address, String command, bool hideDefaultUI) {
    ...
    if (sasConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.sendRC6(data, 20); }
    }

    Serial.println(
        "Sent RC6 Command" + (sasConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
void sendSamsungCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (sasConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.sendSAMSUNG(data, 32); }
    }

    Serial.println(
        "Sent Samsung Command" + (sasConfigPins.irTxRepeats > 0
                                      ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                      : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
void sendSonyCommand(String address, String command, uint8_t nbits, bool hideDefaultUI) {
    ...
    if (sasConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.sendSony(data, nbits, 2); }
    }

    Serial.println(
        "Sent Sony Command" + (sasConfigPins.irTxRepeats > 0
                                   ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                   : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
void sendKaseikyoCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (sasConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.sendPanasonic64(msb_data, 48); }
    }

    Serial.println(
        "Sent Kaseikyo Command" + (sasConfigPins.irTxRepeats > 0
                                       ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                       : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
bool sendDecodedCommand(String protocol, String value, uint8_t bits, bool hideDefaultUI) {
    ...
    if (hasACState(type)) {
        ...
        if (sasConfigPins.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) {
        ...
        if (sasConfigPins.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= sasConfigPins.irTxRepeats; i++) { irsend.send(type, value_int, bits); }
        }
    }

    delay(20);
    Serial.println(
        "Sent Decoded Command" + (sasConfigPins.irTxRepeats > 0
                                      ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                      : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
...
void sendRawCommand(uint16_t frequency, String rawData, bool hideDefaultUI) {
    ...
    Serial.println(
        "Sent Raw Command" + (sasConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(sasConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(sasConfigPins.irTx, LED_OFF);
