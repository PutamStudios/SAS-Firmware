 ...
 *
 * Originally based on work by SAS Lee/EA7KDO with significant enhancements
...
void renderModeSettings(JammerState &state, int &curY, int ySpacing) {
    switch (state.currentMode) {
        case BASIC:
            ...
            tft.setTextColor(
                (state.settingIndex == 3) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor
            );
            padprint("TIMING: ");
        ...
        case ENHANCED_BASIC:
            ...
            tft.setCursor(10, curY);
            tft.setTextColor(
                (state.settingIndex == 4) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor
            );
            padprint("SPACE: ");
            ...
            tft.setCursor(10, curY);
            tft.setTextColor(
                (state.settingIndex == 5) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor
            );
            padprint("POWER: ");
        ...
        case SWEEP:
            ...
            tft.setCursor(10, curY);
            tft.setTextColor(
                (state.settingIndex == 6) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor
            );
            padprint("POWER: ");
            tft.println(String(state.jamDensity));
            break;
...
void displayStats(JammerState &state, int x, int y) {
    ...
    tft.setTextColor(TFT_GREEN, sasConfig.bgColor);
...
void renderJammerUI(JammerState &state) {
    ...
    if (state.redraw) {
        // Clear content area
        tft.fillRect(10, yStart, contentWidth, tftHeight - 55, sasConfig.bgColor);

        // Draw title
        tft.setCursor(10, yStart);
        tft.setTextSize(FM);
        tft.setTextColor(TFT_CYAN, sasConfig.bgColor);
        padprint("IR Jammer");
    ...
    if (state.jamming_active && blinkState) {
        ...
        tft.setTextColor(TFT_MAGENTA, sasConfig.bgColor);
        tft.println("*");
    } else {
        tft.setCursor(tftWidth / 2, tft.getCursorY());
        tft.println(" ");
    ...
    tft.setCursor(10, curY);
    tft.setTextSize(FP);
    tft.setTextColor((state.settingIndex == 0) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor);
    padprint("STATUS: ");
    tft.setTextColor(state.jamming_active ? TFT_RED : TFT_WHITE, sasConfig.bgColor);
    tft.println(state.jamming_active ? "ACTIVE " : "PAUSED ");

    // Display frequency setting
    curY += ySpacing + 10;
    tft.setCursor(10, curY);
    tft.setTextColor((state.settingIndex == 1) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor);
    padprint("FREQ: ");
    ...
    curY += ySpacing;
    tft.setCursor(10, curY);
    tft.setTextColor((state.settingIndex == 2) ? TFT_YELLOW : sasConfig.priColor, sasConfig.bgColor);
    padprint("MODE: ");
...
void performBasicJamming(JammerState &state, IRsend &irsend) {
    ...
    if (currentMillis - state.last_update > 20) {
        ...
        for (int i = 0; i < 50 * state.jamDensity; i++) {
            digitalWrite(sasConfigPins.irTx, HIGH);
            delayMicroseconds(state.markTiming);
            digitalWrite(sasConfigPins.irTx, LOW);
            delayMicroseconds(state.markTiming);
        }
...
void startIrJammer() {
    ...
    IRsend irsend(sasConfigPins.irTx);
