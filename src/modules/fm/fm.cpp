...
void fm_banner() {
    ...
    tft.setCursor(10, 10);
    tft.drawCentreString("~== SAS Radio ==~", tftWidth / 2, 10, SMOOTH_FONT);
    delay(500);
}
    ...
    if (!returnToMenu) {
        ...
        while (!check(EscPress) && !check(SelPress)) {
            ...
            if (noise_level != 0) {
                ...
                tft.fillRect(0, 40, tftWidth, tftHeight, sasConfig.bgColor);
...
bool fm_setup(bool traffic_alert, bool silent) {
    ...
    radio.beginRDS();
    if (traffic_alert) {
        radio.setRDSstation("SASTraffic");
        radio.setRDSbuffer("Traffic Info");
    } else {
        radio.setRDSstation("SASRadio");
        radio.setRDSbuffer("Pwned by SAS Radio!");
    }
