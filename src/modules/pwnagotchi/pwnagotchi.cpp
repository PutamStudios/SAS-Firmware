...
void sasgotchi_setup() {
    initPwngrid();
    initUi();
    state = STATE_INIT;
    Serial.println("SASgotchi Initialized");
}

void sasgotchi_update() {
...
void sasgotchi_start() {
    ...
    if (setupSdCard()) {
        isLittleFS = false;
        if (!SD.exists("/SASPCAP")) SD.mkdir("/SASPCAP");
        if (!SD.exists("/SASPCAP/handshakes")) SD.mkdir("/SASPCAP/handshakes");
        handshakeFs = &SD;
    } else {
        if (!LittleFS.exists("/SASPCAP")) LittleFS.mkdir("/SASPCAP");
        if (!LittleFS.exists("/SASPCAP/handshakes")) LittleFS.mkdir("/SASPCAP/handshakes");
        isLittleFS = true;
    ...
    sasgotchi_setup(); // Starts the thing
    // Draw footer & header
    drawTopCanvas();
    drawBottomCanvas();
