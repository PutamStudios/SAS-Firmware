...
void BLEAttackManager::prepareForConnection() {
    ...
    BLEStateManager::initBLE("SAS-Attack", ESP_PWR_LVL_P9);
...
NimBLEClient *attemptConnectionWithStrategies(NimBLEAddress target, String &connectionMethod) {
    ...
    BLEStateManager::deinitBLE(true);
    delay(800);
    std::string exploitName = "SAS-Exploit";
...
bool MultiConnectionAttack::connectionFloodSingle(NimBLEAddress target, int timeout) {
    BLEStateManager::deinitBLE(true);
    delay(100);
    std::string floodName = "SAS-Flooder";
...
bool MultiConnectionAttack::advertisingSpamSingle(NimBLEAddress target) {
    BLEStateManager::deinitBLE(true);
    delay(300);
    std::string spamName = "SAS-Spammer";
    ...
    uint8_t sasData[] = {0xFF, 0xFF, 'B', 'R', 'U', 'C', 'E'};
    pAdvertising->setManufacturerData(sasData, sizeof(sasData));
    pAdvertising->setName("SAS-Spammer");
...
bool PairingAttackServiceClass::bruteForcePIN(NimBLEAddress target) {
    ...
    for (int i = 0; commonPins[i] != nullptr; i++) {
        ...
        BLEStateManager::deinitBLE(true);
        delay(300);
        std::string pinName = "SAS-PINBrute";
...
String selectFileFromSD() {
    ...
    while (!exitMenu) {
        if (selected != lastSelected || scrollOffset != lastScrollOffset) {
            ...
            tft.setTextColor(TFT_YELLOW, sasConfig.bgColor);
            tft.setCursor(20, 40);
            tft.print("Found: ");
            tft.print(fileCount);
            tft.print(" files");
            ...
            for (int i = 0; i < maxVisibleItems && (scrollOffset + i) < fileCount; i++) {
                ...
                if (fileIdx == selected) {
                    ...
                    tft.fillRect(20, yPos, tftWidth - 40, menuItemHeight - 3, sasConfig.bgColor);
                    tft.setTextColor(TFT_WHITE, sasConfig.bgColor);
                    tft.setCursor(25, yPos + 8);
                    tft.print("  ");
                }
        ...
        if (selectedIdx != lastSelected || scrollOffset != lastScrollOffset) {
            ...
            for (int i = 0; i < maxVisibleDevices && (scrollOffset + i) < deviceCount; i++) {
                ...
                if (i == selectedIdx - scrollOffset) {
                    ...
                    tft.fillRect(15, yPos, tftWidth - 30, deviceItemHeight - 5, sasConfig.bgColor);
                    tft.setTextColor(TFT_WHITE, sasConfig.bgColor);
                    tft.setCursor(20, yPos + 10);
                    tft.print("  ");
                }
...
void showWelcomeScreen() {
    ...
    tft.setCursor((tftWidth - tft.textWidth("SAS")) / 2, 40);
    tft.print("SAS");
        ...
        if (selected != lastSelected || scrollOffset != lastScrollOffset) {
            ...
            for (int i = 0; i < maxVisible && (scrollOffset + i) < MENU_ITEMS; i++) {
                ...
                if (idx == selected) {
                    ...
                    tft.fillRect(20, yPos, tftWidth - 40, 20, sasConfig.bgColor);
                    tft.setTextColor(TFT_WHITE, sasConfig.bgColor);
                    tft.setCursor(25, yPos + 5);
                    tft.print("  ");
                }
...
int showSubMenu(const char *title, const char *options[], int optionCount) {
    ...
    while (true) {
        if (selected != lastSelected || scrollOffset != lastScrollOffset) {
            tft.fillRect(20, 60, tftWidth - 40, tftHeight - 100, sasConfig.bgColor);
...
void showAttackProgress(const char *message, uint16_t color) {
    ...
    tft.setTextColor(color, sasConfig.bgColor);
...
bool requireSimpleConfirmation(const char *message) {
    ...
    tft.fillRect(20, 50, tftWidth - 40, 80, sasConfig.bgColor);
    tft.setCursor(20, 60);
