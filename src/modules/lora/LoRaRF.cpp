...
extern SASConfigPins sasConfigPins;
...
int getLoraIrqPin() {
    ...
    return LORA_IRQ;
#else
    return sasConfigPins.LoRa_bus.io2;
#endif
}
...
int getLoraResetPin() { return sasConfigPins.LoRa_bus.io0; }
int getLoraCsPin() { return sasConfigPins.LoRa_bus.cs; }
...
SPIClass *selectLoraSPIBus() {
    ...
    if (sasConfigPins.LoRa_bus.mosi == TFT_MOSI) {
#if TFT_MOSI > 0
        selectedSPI = &tft.getSPIinstance();
#endif
        Serial.println("Using TFT SPI for LoRa");
    } else if (sasConfigPins.SDCARD_bus.mosi == sasConfigPins.LoRa_bus.mosi) {
        selectedSPI = &sdcardSPI;
        Serial.println("Using SDCard SPI for LoRa");
    } else if (sasConfigPins.NRF24_bus.mosi == sasConfigPins.LoRa_bus.mosi ||
               sasConfigPins.CC1101_bus.mosi == sasConfigPins.LoRa_bus.mosi) {
        selectedSPI = &CC_NRF_SPI;
        CC_NRF_SPI.begin(
            (int8_t)sasConfigPins.LoRa_bus.sck,
            (int8_t)sasConfigPins.LoRa_bus.miso,
            (int8_t)sasConfigPins.LoRa_bus.mosi
        );
        Serial.println("Using CC/NRF SPI for LoRa");
    } else {
        SPI.begin(
            sasConfigPins.LoRa_bus.sck,
            sasConfigPins.LoRa_bus.miso,
            sasConfigPins.LoRa_bus.mosi,
            sasConfigPins.LoRa_bus.cs
        );
...
bool startLoraRadio(float bandMHz) {
    ...
    if (getLoraCsPin() == GPIO_NUM_NC || sasConfigPins.LoRa_bus.mosi == GPIO_NUM_NC ||
        sasConfigPins.LoRa_bus.miso == GPIO_NUM_NC || sasConfigPins.LoRa_bus.sck == GPIO_NUM_NC) {
...
void lorachat() {
    ...
    Serial.println(
        "Pins: SCK:" + String(sasConfigPins.LoRa_bus.sck) +
        " MISO:" + String(sasConfigPins.LoRa_bus.miso) + " MOSI:" + String(sasConfigPins.LoRa_bus.mosi) +
        " CS:" + String(sasConfigPins.LoRa_bus.cs) + " RST:" + String(getLoraResetPin()) +
