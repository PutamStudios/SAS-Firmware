...
void ReverseShell() {
    ...
    if (!WiFi.softAP("SASShell", "", 1)) {
        tft.println("Failed to start AP");
        return;
    }

    tft.println("Wi-Fi AP Started: SASShell");
    ...
    webServer.on("/", [&webServer]() {
        String html = R"rawliteral(
            ...
            <html>
            <head>
                <title>SASShell Web Interface</title>
                <style>
    ...
    while (true) {
        ...
        if (!shellConnected) {
            ...
            if (tcpClient) {
                ...
                tcpClient.println("~Welcome to SASShell.");
