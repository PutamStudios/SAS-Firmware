...
static SPIClass *selectEthernetSPIBus() {
    ...
    if (sasConfigPins.W5500_bus.mosi == (gpio_num_t)TFT_MOSI &&
        sasConfigPins.W5500_bus.mosi != GPIO_NUM_NC) {
#if TFT_MOSI > 0
...
#endif
    } else if (sasConfigPins.W5500_bus.mosi == sasConfigPins.SDCARD_bus.mosi &&
               sasConfigPins.W5500_bus.mosi != GPIO_NUM_NC) {
        selectedSPI = &sdcardSPI;
        sdcardSPI.begin(
            (int8_t)sasConfigPins.W5500_bus.sck,
            (int8_t)sasConfigPins.W5500_bus.miso,
            (int8_t)sasConfigPins.W5500_bus.mosi,
            (int8_t)sasConfigPins.W5500_bus.cs
        );
        Serial.println("Using SDCard SPI for Ethernet");
    } else if ((sasConfigPins.W5500_bus.mosi == sasConfigPins.NRF24_bus.mosi ||
                sasConfigPins.W5500_bus.mosi == sasConfigPins.CC1101_bus.mosi) &&
               sasConfigPins.W5500_bus.mosi != GPIO_NUM_NC) {
...
bool EthernetHelper::setup() {
    if (sasConfigPins.W5500_bus.cs == GPIO_NUM_NC || sasConfigPins.W5500_bus.sck == GPIO_NUM_NC ||
        sasConfigPins.W5500_bus.miso == GPIO_NUM_NC || sasConfigPins.W5500_bus.mosi == GPIO_NUM_NC) {
    ...
    const int csPin = static_cast<int>(sasConfigPins.W5500_bus.cs);
    const int irqPin =
        (sasConfigPins.W5500_bus.io0 == GPIO_NUM_NC) ? -1 : static_cast<int>(sasConfigPins.W5500_bus.io0);
    const int rstPin =
        (sasConfigPins.W5500_bus.io2 == GPIO_NUM_NC) ? -1 : static_cast<int>(sasConfigPins.W5500_bus.io2);
