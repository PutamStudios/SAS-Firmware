...
bool ARPSpoofer::arpPCAPfile() {
    ...
    while (fs->exists(String("/SASPCAP/ARP_session_" + String(nf++) + ".pcap").c_str())) yield();
    pcapFile = fs->open(String("/SASPCAP/ARP_session_" + String(nf) + ".pcap").c_str(), FILE_WRITE);
...
void ARPSpoofer::setup(const Host &host, IPAddress gateway) {
    ...
    if (mitm) {
        ...
        // padprintln("/SASPCAP/ARP_session_" + String(nf) + ".pcap");
