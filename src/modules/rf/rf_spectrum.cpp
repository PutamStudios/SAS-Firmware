...
void draw_tf_spectrum_grid() {
    ...
    tft.printf(" RF - Spectrum (%.2f Mhz)", sasConfigPins.rfFreq);
    tft.fillRect(0, 20, tftWidth, tftHeight - 20, sasConfig.bgColor);
    ...
    while (1) {
        ...
        if (rx_size != 0) {
            ...
            for (size_t i = 0; i < rx_size; i++) {
                ...
                tft.drawLine(lineX, startY, lineX, endY, sasConfig.priColor);
            }
...
void rf_SquareWave() {
    RCSwitch rcswitch;
    if (!initRfModule("rx", sasConfigPins.rfFreq)) return;

    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) rcswitch.enableReceive(sasConfigPins.CC1101_bus.io0);
    else rcswitch.enableReceive(sasConfigPins.rfRx);
    int line_w = 0;
    int line_h = 15;
    unsigned int *raw;
PRINT:
    ...
    tft.setTextSize(1);
    tft.setCursor(3, 2);
    tft.printf("  RF - SquareWave (%.2f Mhz)", sasConfigPins.rfFreq);

    while (1) {
        if (rcswitch.RAWavailable()) {
            raw = rcswitch.getRAWReceivedRawdata();
            // Clear the display area
            // tft.fillRect(0, 0, tftWidth, tftHeight, sasConfig.bgColor);
            // Draw waveform based on signal strength
            for (int i = 0; i < RCSWITCH_RAW_MAX_CHANGES - 1; i += 2) {
                ...
                if (line_h > tftHeight) {
                    line_h = 15;
                    tft.fillRect(0, 12, tftWidth, tftHeight, sasConfig.bgColor);
                }
                tft.drawFastVLine(line_w, line_h, 6, sasConfig.priColor);
                tft.drawFastHLine(line_w, line_h, raw[i] / TIME_DIVIDER, sasConfig.priColor);

                tft.drawFastVLine(line_w + raw[i] / TIME_DIVIDER, line_h, 6, sasConfig.priColor);
                tft.drawFastHLine(
...
void rf_CC1101_rssi() {
    ...
    while (1) {
        if (redraw) {
            ...
            if (sasConfigPins.rfFxdFreq) {
                if (!initRfModule("rx", sasConfigPins.rfFreq))
                    displayError("Error setting frequency", true);
                tft.printf(" RF - RSSI spectrum (%.2f Mhz)", sasConfigPins.rfFreq);
                tft.drawFastVLine(20, 20, tftHeight, sasConfig.priColor);
            ...
            // Range Scan Sees a bargraph simillar to NRF24 grafic, using RSSI across frequencies
                ...
                if (!initRfModule("rx", sasConfigPins.rfFreq)) displayError("Error starting module", true);
                tft.printf(" RF - RSSI spectrum (%s)", subghz_frequency_ranges[sasConfigPins.rfScanRange]);
                tft.drawFastHLine(0, tftHeight - 20, tftWidth, sasConfig.priColor);
                char buf[7];
                float var = subghz_frequency_list[range_limits[sasConfigPins.rfScanRange][0]];
                snprintf(buf, sizeof(buf), "%.3f", var);
                tft.drawString(buf, 2, tftHeight - 10);
                var = subghz_frequency_list[range_limits[sasConfigPins.rfScanRange][1]];
                snprintf(buf, sizeof(buf), "%.3f", var);
                tft.drawRightString(buf, tftWidth - 2, tftHeight - 10);
                int range = range_limits[sasConfigPins.rfScanRange][1] -
                            range_limits[sasConfigPins.rfScanRange][0] + 1;
                int space = tftWidth / range;
                for (int i = 0; i < range; i++) {
                    tft.drawFastVLine(space * i, tftHeight - 20, 5, sasConfig.priColor);
                }
        ...
        if (sasConfigPins.rfFxdFreq) {
            ...
            for (int i = 1; i < graph_size; i++) {
                ...
                tft.drawLine(x0, base_y - prev, x1, base_y - curr, sasConfig.bgColor);
                ...
                tft.drawLine(x0, base_y - curr, x1, base_y - next_val, sasConfig.priColor);
                prev = curr;
            }
            tft.drawFastVLine(20, 20, tftHeight, sasConfig.priColor);
        ...
        // draw a bargraph similar to nrf24 across the range
            ...
            for (int i = 0; i < range; i++) {
                if (EscPress || SelPress) break;
                setMHZ(subghz_frequency_list[range_limits[sasConfigPins.rfScanRange][0] + i]);
                ...
                tft.fillRect(
                    i * space, tftHeight - 20 - bar_size[i], space - 2, bar_size[i], sasConfig.priColor
                );
                tft.fillRect(i * space, 20, space, max_bar_size - bar_size[i], sasConfig.bgColor);
                if (bar_size[i] > bar_size[max_idx] && bar_size[i] > min_value) max_idx = i;
            }
            if (bar_size[max_idx] > min_value) {
                char buf[7];
                float var = subghz_frequency_list[range_limits[sasConfigPins.rfScanRange][0] + max_idx];
        ...
        if (check(SelPress)) {
            deinitRfModule();
            rf_range_selection(sasConfigPins.rfFreq);
            redraw = true;
        }
    }
    deinitRfModule();
