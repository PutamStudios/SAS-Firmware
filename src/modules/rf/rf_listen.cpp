...
void IRAM_ATTR onPulse() {
    ...
    if (digitalRead(sasConfigPins.CC1101_bus.io0)) {
...
void rf_listen() {
    ...
    while (!check(SelPress) && !check(EscPress)) {
        ...
        if (redraw) {
            ...
            displayRedStripe(text, getComplementaryColor2(sasConfig.priColor), sasConfig.priColor);
        }
    ...
    attachInterrupt(digitalPinToInterrupt(sasConfigPins.CC1101_bus.io0), onPulse, CHANGE);
    displayRedStripe("Listening...", getComplementaryColor2(sasConfig.priColor), sasConfig.priColor);
    ...
    while (!check(EscPress)) {
        displayRedStripe(
            "Waiting for a pulse", getComplementaryColor2(sasConfig.priColor), sasConfig.priColor
        );
        if (newPulse) {
            newPulse = false;
            ...
            displayRedStripe(pulseText, getComplementaryColor2(sasConfig.priColor), sasConfig.priColor);
#if defined(BUZZ_PIN)
        ...
        if (pulseActive && millis() - lastPulseTime > 3000) {
            pulseActive = false;
            displayRedStripe("No signal", getComplementaryColor2(sasConfig.priColor), sasConfig.priColor);
        }
    ...
    detachInterrupt(digitalPinToInterrupt(sasConfigPins.CC1101_bus.io0));
}
