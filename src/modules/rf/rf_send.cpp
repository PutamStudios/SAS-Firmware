...
void sendCustomRF() {
    ...
    while (!returnToMenu) {
        ...
        num_signal_repeat = 4;
        delay(200);
        filepath = loopSD(*filesystem, true, "SUB", "/SASRF");
...
void sendRfCommand(struct RfCodes rfcode, bool hideDefaultUI) {
    ...
    */
    ...
    // digitalWrite(sasConfigPins.rfTx, LED_OFF);
    deinitRfModule();
}
...
void RCSwitch_send(uint64_t data, unsigned int bits, int pulse, int protocol, int repeat) {
    ...
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        mySwitch.enableTransmit(sasConfigPins.CC1101_bus.io0);
    } else {
        mySwitch.enableTransmit(sasConfigPins.rfTx);
    }
...
void RCSwitch_RAW_Bit_send(RfCodes data) {
    int nTransmitterPin = sasConfigPins.rfTx;
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) { nTransmitterPin = sasConfigPins.CC1101_bus.io0; }
