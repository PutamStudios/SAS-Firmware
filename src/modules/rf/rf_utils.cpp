...
bool initRfModule(String mode, float frequency) {
    ...
    if (!frequency) frequency = sasConfigPins.rfFreq;

    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) { // CC1101 in use
        if (sasConfigPins.CC1101_bus.mosi == (gpio_num_t)TFT_MOSI &&
            sasConfigPins.CC1101_bus.mosi != GPIO_NUM_NC) { // (T_EMBED), CORE2 and others
#if TFT_MOSI > 0
            initCC1101once(&tft.getSPIinstance());
#else
            yield();
#endif
        } else if (sasConfigPins.CC1101_bus.mosi ==
                   sasConfigPins.SDCARD_bus.mosi) { // (CARDPUTER) and (ESP32S3DEVKITC1) and devices that
                                                      // share CC1101 pin with only SDCard
            initCC1101once(&sdcardSPI);
        } else if (sasConfigPins.NRF24_bus.mosi == sasConfigPins.CC1101_bus.mosi &&
                   sasConfigPins.CC1101_bus.mosi !=
                       sasConfigPins.SDCARD_bus
            ...
            delay(10);
            if (!CC_NRF_SPI.begin(
                    sasConfigPins.CC1101_bus.sck,
                    sasConfigPins.CC1101_bus.miso,
                    sasConfigPins.CC1101_bus.mosi
                )) {
            ...
            // change it when SAS board comes to shore)
        ...
        if (mode == "tx") {
            ...
            pinMode(sasConfigPins.CC1101_bus.io0, OUTPUT);
            ...
            pinMode(sasConfigPins.CC1101_bus.io0, INPUT);
        ...
        if (abs(frequency - sasConfigPins.rfFreq) > 1) {
        ...
        if (mode == "tx") {
            gsetRfTxPin(false);
            if (sasConfigPins.SDCARD_bus.checkConflict(sasConfigPins.rfTx)) sdcardSPI.end();
            gpio_reset_pin((gpio_num_t)sasConfigPins.rfTx);
            pinMode(sasConfigPins.rfTx, OUTPUT);
            digitalWrite(sasConfigPins.rfTx, LOW);

        } else if (mode == "rx") {
            // Rx Mode
            gsetRfRxPin(false);
            if (sasConfigPins.SDCARD_bus.checkConflict(sasConfigPins.rfRx)) sdcardSPI.end();
            gpio_reset_pin((gpio_num_t)sasConfigPins.rfRx);
            pinMode(sasConfigPins.rfRx, INPUT);
        }
    }
    // no error
    return true;
...
void deinitRfModule() {
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        if (cc1101_spi_ready) {
            ELECHOUSE_cc1101.setSidle();
            cc1101_spi_ready = false;
        }
        digitalWrite(sasConfigPins.CC1101_bus.io0, LOW);
        digitalWrite(sasConfigPins.CC1101_bus.cs, HIGH);
        ioExpander.turnPinOnOff(IO_EXP_CC_RX, LOW);
        ioExpander.turnPinOnOff(IO_EXP_CC_TX, LOW);
    } else digitalWrite(sasConfigPins.rfTx, LED_OFF);
}
...
void initCC1101once(SPIClass *SSPI) {
    ...
    ELECHOUSE_cc1101.setSpiPin(
        ...
        sasConfigPins.CC1101_bus.miso,
        sasConfigPins.CC1101_bus.mosi,
        sasConfigPins.CC1101_bus.cs
    );
    ELECHOUSE_cc1101.setGDO0(sasConfigPins.CC1101_bus.io0); // use Gdo0 for both Tx and Rx
...
void setMHZ(float frequency) {
    ...
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        ...
        if (sasConfigPins.CC1101_bus.cs != 17) change = false;
#endif
...
rmt_channel_handle_t setup_rf_rx() {
    if (!initRfModule("rx", sasConfigPins.rfFreq)) return NULL;
    setMHZ(sasConfigPins.rfFreq);
    rmt_rx_channel_config_t rx_channel_cfg = {};
    rx_channel_cfg.gpio_num = sasConfigPins.rfModule == CC1101_SPI_MODULE
                                  ? gpio_num_t(sasConfigPins.CC1101_bus.io0)
                                  : gpio_num_t(sasConfigPins.rfRx); // GPIO number
...
bool setMHZMenu() {
    if (sasConfigPins.rfModule != CC1101_SPI_MODULE) return false;
    if (check(SelPress)) {
        options = {};
        int ind = 0;
        int arraySize = sizeof(subghz_frequency_list) / sizeof(subghz_frequency_list[0]);
        for (int i = 0; i < arraySize; i++) {
            if (subghz_frequency_list[i] - sasConfigPins.rfFreq < 0.1) ind = i;
            String tmp = String(subghz_frequency_list[i], 2) + "Mhz";
            options.push_back({tmp.c_str(), [=]() { sasConfigPins.rfFreq = subghz_frequency_list[i]; }});
        }
...
void rf_range_selection(float currentFrequency) {
    int option = 0;
    options = {
        {String("Fixed [" + String(sasConfigPins.rfFreq) + "]").c_str(),
         [=]() { sasConfigPins.setRfFreq(sasConfigPins.rfFreq, 2); }                                               },
        {String("Choose Fixed").c_str(),                                   [&]() { option = 1; }                       },
        {subghz_frequency_ranges[0],                                       [=]() { sasConfigPins.setRfScanRange(0); }},
        {subghz_frequency_ranges[1],                                       [=]() { sasConfigPins.setRfScanRange(1); }},
        {subghz_frequency_ranges[2],                                       [=]() { sasConfigPins.setRfScanRange(2); }},
        {subghz_frequency_ranges[3],                                       [=]() { sasConfigPins.setRfScanRange(3); }},
    };
    ...
    if (option == 1) { // Fixed Frequency Selector
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            options.push_back({tmp.c_str(), [=]() {
                                   sasConfigPins.setRfFreq(subghz_frequency_list[i], 2);
                               }});
    ...
    if (sasConfigPins.rfFxdFreq) displayTextLine("Scan freq set to " + String(sasConfigPins.rfFreq));
    else displayTextLine("Range set to " + String(subghz_frequency_ranges[sasConfigPins.rfScanRange]));
}
