...
void sinewave_animation() {
    ...
    for (int x = 20; x < tftWidth - 20; x++) {
        ...
        tft.drawFastVLine(x, lastY, sinewaveWidth, sasConfig.bgColor);
        tft.drawFastVLine(x, y, sinewaveWidth, sasConfig.priColor);
    }
...
void rf_raw_record_draw(RawRecordingStatus status) {
    ...
    if (status.frequency <= 0) {
        ...
        // Draw the bar
        tft.drawFastVLine(x, yTop, barHeight * 2, sasConfig.priColor);
    }
}
...
float rf_freq_scan() {
    float frequency = 0;
    int idx = range_limits[sasConfigPins.rfScanRange][0];
    uint8_t attempt = 0;
        ...
        if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
            ...
            if (rssi > rssiThreshold) {
                ...
                if (attempt >= FREQUENCY_SCAN_MAX_TRIES) {
                    ...
                    sasConfigPins.setRfFreq(best_frequencies[max_index].freq, 0);
            ...
            frequency = 433.92;
            sasConfigPins.setRfFreq(433.92, 2);
        }
    }
    return frequency;
}
...
void rf_raw_record_create(RawRecording &recorded, bool &returnToMenu) {
    ...
    rssiFeature = sasConfigPins.rfModule == CC1101_SPI_MODULE;

    tft.fillScreen(sasConfig.bgColor);
    drawMainBorder();

    if (rssiFeature) rf_range_selection(sasConfigPins.rfFreq);
    ...
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        if (sasConfigPins.rfFxdFreq || !rssiFeature) status.frequency = sasConfigPins.rfFreq;
        else status.frequency = rf_freq_scan();
    } else status.frequency = sasConfigPins.rfFreq;
    ...
    tft.drawPixel(0, 0, 0);
    tft.fillRect(10, 30, tftWidth - 20, tftHeight - 40, sasConfig.bgColor);
    rf_raw_record_draw(status);
