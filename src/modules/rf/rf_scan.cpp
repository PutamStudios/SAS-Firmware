...
void RFScan::setup() {
    ...
    if (sasConfigPins.rfScanRange < 0 || sasConfigPins.rfScanRange > 3) {
        sasConfigPins.setRfScanRange(3);
    }
    if (sasConfigPins.rfModule != CC1101_SPI_MODULE) { sasConfigPins.setRfFxdFreq(1); }

    display_info(received, signals, ReadRAW, codesOnly, autoSave, title);

    if (sasConfigPins.rfFxdFreq) frequency = sasConfigPins.rfFreq;
...
void RFScan::RCSwitch_Enable_Receive(RCSwitch rcswitch) {
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE) {
        rcswitch.enableReceive(sasConfigPins.CC1101_bus.io0);
    } else {
        rcswitch.enableReceive(sasConfigPins.rfRx);
    }
}
...
bool RFScan::fast_scan() {
    ...
    if (idx < range_limits[sasConfigPins.rfScanRange][0] ||
        idx > range_limits[sasConfigPins.rfScanRange][1]) {
        idx = range_limits[sasConfigPins.rfScanRange][0];
    }
    ...
    if (rssi > rssiThreshold) {
        ...
        if (_try >= _MAX_TRIES) {
            ...
            sasConfigPins.setRfFreq(_freqs[max_index].freq, 2); // change to fixed frequency
...
void RFScan::select_menu_option() {
    ...
    if (sasConfigPins.rfModule == CC1101_SPI_MODULE && !sasConfigPins.rfFxdFreq)
...
void RFScan::set_range() {
    ...
    options = {
        {String("Fxd [" + String(sasConfigPins.rfFreq) + "]").c_str(),
         [=]() { sasConfigPins.setRfScanRange(sasConfigPins.rfScanRange, 1); } },
        {"Choose Fxd",                                                   [&]() { chooseFixedOpt = true; } },
        {subghz_frequency_ranges[0],                                     [=]() {
sasConfigPins.setRfScanRange(0); }}, {subghz_frequency_ranges[1],                                     [=]()
{ sasConfigPins.setRfScanRange(1); }}, {subghz_frequency_ranges[2], [=]() {
sasConfigPins.setRfScanRange(2); }}, {subghz_frequency_ranges[3],                                     [=]()
{ sasConfigPins.setRfScanRange(3); }},
    };
    ...
    if (chooseFixedOpt) { // Range
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            options.emplace_back(tmp.c_str(), [=]() { sasConfigPins.rfFreq = subghz_frequency_list[i]; });
...
void display_info(RfCodes received, int signals, bool ReadRAW, bool codesOnly, bool autoSave, String title) {
    ...
    if (sasConfigPins.rfFxdFreq) padprintln("Scanning: " + String(sasConfigPins.rfFreq) + " MHz");
    else padprintln("Scanning: " + String(subghz_frequency_ranges[sasConfigPins.rfScanRange]));
...
bool RCSwitch_SaveSignal(float frequency, RfCodes codes, bool raw, char *key, bool autoSave) {
    ...
    String subfile_out = "Filetype: SAS SubGhz File\nVersion 1\n";
...
String RCSwitch_Read(float frequency, int max_loops, bool raw, bool headless) {
    ...
    if (!frequency) frequency = sasConfigPins.rfFreq; // default from config
