...
RF24 NRFradio(sasConfigPins.NRF24_bus.io0, sasConfigPins.NRF24_bus.cs);
...
bool nrf_start(NRF24_MODE mode) {
    ...
    if (CHECK_NRF_UART(mode)) {
        ...
        NRFSerial.begin(115200, SERIAL_8N1, sasConfigPins.uart_bus.rx, sasConfigPins.uart_bus.tx);
        Serial.println("NRF24 on Serial Started");
        result = true;
    };

    if (!CHECK_NRF_SPI(mode)) return result;
    pinMode(sasConfigPins.NRF24_bus.cs, OUTPUT);
    digitalWrite(sasConfigPins.NRF24_bus.cs, HIGH);
    pinMode(sasConfigPins.NRF24_bus.io0, OUTPUT);
    digitalWrite(sasConfigPins.NRF24_bus.io0, LOW);

    if (sasConfigPins.NRF24_bus.mosi == (gpio_num_t)TFT_MOSI &&
        sasConfigPins.NRF24_bus.mosi != GPIO_NUM_NC) { // (T_EMBED), CORE2 and others
    ...
    } else if (sasConfigPins.NRF24_bus.mosi == sasConfigPins.SDCARD_bus.mosi) {
        // CC1101 shares SPI with SDCard (Cardputer and CYDs)

        NRFSPI = &sdcardSPI;
    } else if (sasConfigPins.NRF24_bus.mosi == sasConfigPins.CC1101_bus.mosi &&
               sasConfigPins.NRF24_bus.mosi != sasConfigPins.SDCARD_bus.mosi) {
    ...
    NRFSPI->begin(
        (int8_t)sasConfigPins.NRF24_bus.sck,
        (int8_t)sasConfigPins.NRF24_bus.miso,
        (int8_t)sasConfigPins.NRF24_bus.mosi
    );
    delay(10);

    if (NRFradio.begin(
            NRFSPI,
            rf24_gpio_pin_t(sasConfigPins.NRF24_bus.io0),
            rf24_gpio_pin_t(sasConfigPins.NRF24_bus.cs)
        )) {
        result = true;
    } else {
        return false;
