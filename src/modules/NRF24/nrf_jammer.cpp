...
static void editModeConfig(NrfJamMode mode) {
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            for (int i = 0; i < 5; i++) {
                int itemY = y + i * lineH;
                uint16_t fg = (i == menuIdx) ? sasConfig.bgColor : sasConfig.priColor;
                uint16_t bg = (i == menuIdx) ? sasConfig.priColor : sasConfig.bgColor;
...
static void drawJammerStatus(NrfJamMode mode, int currentCh, uint8_t nrfOnline, bool initial) {
    ...
    // Mode name
    tft.fillRect(7, y, tftWidth - 14, lineH, sasConfig.bgColor);
    ...
    // Config summary
    tft.fillRect(7, y, tftWidth - 14, lineH, sasConfig.bgColor);
    tft.setTextColor(TFT_DARKGREY, sasConfig.bgColor);
    ...
    tft.fillRect(7, footerY, tftWidth - 14, FP * LH, sasConfig.bgColor);
...
void nrf_channel_jammer() {
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            // Pause state
            tft.setTextColor(paused ? TFT_RED : TFT_GREEN, sasConfig.bgColor);
            tft.fillRect(7, contentY, tftWidth - 14, lineH, sasConfig.bgColor);
...
void nrf_channel_hopper() {
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            for (int i = 0; i < 5; i++) {
                int itemY = y + i * lineH;
                uint16_t fg = (i == menuIndex) ? sasConfig.bgColor : sasConfig.priColor;
                uint16_t bg = (i == menuIndex) ? sasConfig.priColor : sasConfig.bgColor;
            ...
            if (menuIndex == 3) {
                ...
                while (true) {
                    ...
                    if (hopRedraw) {
                        ...
                        tft.fillRect(7, contentY, tftWidth - 14, lineHop, sasConfig.bgColor);
                        tft.setTextColor(TFT_GREEN, sasConfig.bgColor);
                        char hBuf[40];
                        snprintf(
                            hBuf,
