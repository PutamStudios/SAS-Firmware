 ...
 * @brief Enhanced 2.4 GHz spectrum analyzer for SAS firmware.
 *
 * Features:
...
String scanChannels(bool web) {
    ...
    digitalWrite(sasConfigPins.NRF24_bus.io0, HIGH);
    ...
    for (int i = 0; i < NRF_SPECTRUM_CHANNELS; i++) {
        ...
        uint16_t gridColor = (i % 10 == 0) ? TFT_DARKGREY : sasConfig.bgColor;
    ...
    if (maxLevel > 10) {
        ...
        int pkW = 42;
        int pkY = 1;
        tft.fillRect(tftWidth - pkW - spec_marginL, pkY, pkW, 10, sasConfig.bgColor);
        ...
        for (int i = 0; i < NRF_SPECTRUM_CHANNELS; i++) {
            ...
            if ((channel[i] > 10) || (deviceLabelTimer[i] > 0)) {
                ...
                if (dev != DEV_NONE) {
                    ...
                    tft.setTextSize(FP);
                    tft.setTextColor(deviceInfo[dev].labelColor, sasConfig.bgColor);
    ...
    if (nrf_start(NRF_MODE_SPI)) {
        ...
        while (!check(EscPress)) {
            ...
            if (check(SelPress)) {
                ...
                tft.fillRect(0, spec_barAreaY, tftWidth, spec_barAreaH, sasConfig.bgColor);
                delay(200);
            }
        }
