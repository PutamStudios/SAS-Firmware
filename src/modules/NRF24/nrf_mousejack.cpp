 ...
 * @brief MouseJack scan, fingerprint, and HID injection for SAS firmware.
 *
 ...
 *          adapted for SAS architecture with multi-platform input.
 */
...
static void mj_drawScanScreen(uint8_t currentCh, bool initial) {
    ...
    tft.setTextSize(FP);
    tft.fillRect(7, contentY, tftWidth - 14, 12, sasConfig.bgColor);
    ...
    for (int i = 0; i < maxItems && i < mj_targetCount; i++) {
        int y = listY + i * 12;
        tft.fillRect(7, y, tftWidth - 14, 12, sasConfig.bgColor);
        char line[40];
        snprintf(
            line,
            sizeof(line),
