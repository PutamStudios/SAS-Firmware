...
void pix_qrcode() {
    ...
    String pix_code = "00020126" + String(data0.length()) + data0 + "52040000530398654" + amount_length +
                      amount + "5802BR5909SAS PIX6014Rio de Janeiro62070503***6304";
...
void qrcode_menu() {
    ...
    for (const auto &entry : sasConfig.qrCodes) {
...
void save_and_display_qrcode() {
    ...
    if (std::any_of(
            sasConfig.qrCodes.begin(),
            sasConfig.qrCodes.end(),
            [&](const SASConfig::QrCodeEntry &entry) { return entry.menuName == name; }
        )) {
    ...
    sasConfig.addQrCodeEntry(name, text);
    return qrcode_display(text);
}

void remove_custom_qrcode() {
    if (sasConfig.qrCodes.empty()) {
    ...
    for (const auto &entry : sasConfig.qrCodes) {
        options.emplace_back(entry.menuName.c_str(), [=]() {
            sasConfig.removeQrCodeEntry(entry.menuName);
