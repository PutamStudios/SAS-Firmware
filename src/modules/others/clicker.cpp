...
void drawMenuItem(
    ...
    tft.fillRect(
        layout.margin - 5, yPos - 5, tftWidth - 2 * layout.margin + 10, clearHeight + 10, sasConfig.bgColor
    );
    ...
    if (isSelected && itemIndex != ITEM_START) {
        uint16_t borderColor = isEdit ? TFT_GREEN : sasConfig.priColor;
    ...
    switch (itemIndex) {
        case ITEM_DELAY: {
            ...
            if (isEdit && isSelected) tft.setTextColor(TFT_YELLOW, sasConfig.bgColor);
            tft.print(delayStr);
...
void drawConfigScreen(const LayoutConfig &layout) {
    ...
    tft.fillRect(0, 0, tftWidth, layout.header_height, sasConfig.priColor);
    tft.setTextColor(sasConfig.bgColor, sasConfig.priColor);
    tft.setTextSize(layout.text_size_large);
...
void updateCPSDisplay(const LayoutConfig &layout, int currentCPS, unsigned long totalClicks) {
    ...
    tft.fillRect(0, cpsY - 5, tftWidth, cpsSize * 8 + 30, sasConfig.bgColor);
...
void drawUSBInitScreen(const LayoutConfig &layout) {
    ...
    for (int i = 3; i > 0; i--) {
        ...
        tft.fillRect(0, numY - 5, tftWidth, 30, sasConfig.bgColor);
...
void clicker_setup() {
    ...
    tft.fillRect(0, promptY - 5, tftWidth, 25, sasConfig.bgColor);
