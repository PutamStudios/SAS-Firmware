...
void mic_test_one_task() {
    ...
    tft.drawRect(displayX - 2, displayY - 2, displayWidth + 4, displayHeight + 4, sasConfig.priColor);
...
void mic_record_app() {
    ...
    auto drawItem = [&](int itemIndex, bool isSelected, bool isEdit) {
        ...
        tft.fillRect(MARGIN - 5, yPos - 5, tftWidth - 2 * MARGIN + 10, clearHeight + 10, sasConfig.bgColor);
    ...
    // Header
    tft.fillRect(0, 0, tftWidth, HEADER_HEIGHT, sasConfig.priColor);
    ...
    // ===== RECORDING BLOCK =====
    {
        ...
        int index = 0;
        do {
            snprintf(filename, sizeof(filename), "/SASMIC/recording_%d.wav", index++);
            ...
            if (!stealth_enabled) {
                if (millis() - lastUpdate > 500) {
                    ...
                    if (String(timerStr) != lastTimerStr) {
                        ...
                        tft.fillRect(0, TIMER_Y - 5, tftWidth, TIMER_SIZE * 8 + 10, sasConfig.bgColor);
