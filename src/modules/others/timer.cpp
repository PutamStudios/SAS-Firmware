...
void Timer::clearUnderline() {
    tft.drawLine(BORDER_PAD_X, underlineY, tftWidth - BORDER_PAD_X, underlineY, sasConfig.bgColor);
}
...
void Timer::underlineMinutes() {
    tft.drawLine(
        timerX - (LW * fontSize), underlineY, timerX + (LW * fontSize), underlineY, sasConfig.priColor
    );
}
...
void Timer::drawSoundOption(bool highlight) {
    ...
    uint16_t textColor = highlight ? sasConfig.priColor : TFT_DARKGREY;
    uint16_t statusColor = playSoundOnFinish ? TFT_GREEN : TFT_RED;

    // Clear the line first
    tft.fillRect(BORDER_PAD_X, optionY, tftWidth - BORDER_PAD_X * 2, LH + 2, sasConfig.bgColor);

    // Build the option text
    char optionText[32];
    snprintf(optionText, sizeof(optionText), "Play sound: %s", playSoundOnFinish ? "ON" : "OFF");

    tft.setTextColor(textColor, sasConfig.bgColor);
    ...
    if (highlight) {
        ...
        tft.drawLine(startX, indicatorY, endX, indicatorY, sasConfig.priColor);
    }
}
