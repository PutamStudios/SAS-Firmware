...
void initSprites() {
    ...
    sprite.fillScreen(sasConfig.bgColor);
    ...
    sprite.fillRect(0, 21, 32, 15, sasConfig.bgColor);

    // draw para desenhar o peixe
    draw.deleteSprite();
    draw.createSprite(20, 8);
    draw.fillScreen(sasConfig.bgColor);
...
void moveFish(Fish &f) {
    ...
    if (f.x < -10) {
        tft.fillRect(f.x, f.y, 22, 11, sasConfig.bgColor);
...
void checkCollisions() {
    for (int i = 0; i < 5; i++) {
        if ((sharkX < fish[i].x + fish[i].size) && (sharkX + sharkSize > fish[i].x) &&
            ...
            tft.fillRect(fish[i].x, fish[i].y, 18, 8, sasConfig.bgColor);
