...
private:
    ...
    bool ensureAttestationCert(const uint8_t attPriv[32]) {
        ...
        do {
            ...
            if (rc != 0) break;
            rc = mbedtls_x509write_crt_set_subject_name(&crt, "CN=SAS U2F Attestation,O=SASDevices,C=US");
            if (rc != 0) break;
            rc = mbedtls_x509write_crt_set_issuer_name(&crt, "CN=SAS U2F Attestation,O=SASDevices,C=US");
            if (rc != 0) break;
...
void updateU2fRuntimeInfo(const U2fHidDevice &device) {
    tft.fillRect(0, 84, tftWidth, tftHeight - 84, sasConfig.bgColor);
    tft.setTextSize(2);
    tft.setCursor(6, 94);
