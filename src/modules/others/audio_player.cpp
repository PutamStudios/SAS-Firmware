...
void drawButton(int x, int y, int size, IconType icon, bool selected, bool active) {
    ...
    uint16_t borderColor = selected ? sasConfig.priColor : TFT_DARKGREY;
...
void drawProgressBar(int x, int y, int width, int height, unsigned long position, unsigned long duration) {
    ...
    uint16_t barFill = sasConfig.priColor;
...
bool showVolumeControl(uint8_t &currentVolume) {
    ...
    auto drawVolumeBar = [&]() {
        ...
        uint16_t volColor = (tempVolume > 80) ? TFT_RED : sasConfig.priColor;
...
void musicPlayerUI(FS *fs, const String &filepath) {
    ...
    uint8_t currentVolume = sasConfig.soundVolume;

    // --- Draw functions ---

    auto drawHeader = [&]() {
        tft.fillRect(0, 0, tftWidth, ui.HEADER_HEIGHT, sasConfig.priColor);
    ...
    auto drawTrackInfo = [&]() {
        ...
        tft.fillRect(0, y, tftWidth, ui.DISPLAY_HEIGHT, sasConfig.bgColor);
    ...
    auto drawTimers = [&]() {
        ...
        tft.setTextColor(TFT_LIGHTGREY, sasConfig.bgColor);
            ...
            if (selectedButton == BTN_PLAY && millis() - player.lastPauseAction < 300) {
                ...
                switch (selectedButton) {
                    ...
                    case BTN_VOLUME:
                        ...
                        tft.fillScreen(sasConfig.bgColor); // Full redraw after popup
                        drawHeader();
