...
static AudioOutputI2S *createConfiguredAudioOutput() {
    ...
    audioout->SetGain(sasConfig.soundVolume / AUDIO_VOLUME_MAX);
...
AudioPlaybackInfo getAudioPlaybackInfo() {
    ...
    info.duration = 0;
    info.position = 0;
    info.volume = sasConfig.soundVolume;
...
void setAudioPlaybackVolume(uint8_t volume) {
    ...
    // Update config
    sasConfig.setSoundVolume(volume);
...
static bool startAsyncPlayback(
    ...
    g_audioPlayer->currentGain = sasConfig.soundVolume / AUDIO_VOLUME_MAX; // Store initial gain
...
bool playAudioFile(FS *fs, String filepath, PlaybackMode mode) {
    if (!sasConfig.soundEnabled) return false;
...
void playTone(unsigned int frequency, unsigned long duration, short waveType) {
    if (!sasConfig.soundEnabled) return;
    ...
    float volumeScale = (sasConfig.soundVolume / AUDIO_VOLUME_MAX) * AUDIO_VOLUME_SCALE;
