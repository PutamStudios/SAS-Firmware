...
void ducky_startKb(HIDInterface *&hid, bool ble) {
    ...
    if (hid == nullptr) {
        ...
        if (ble) {
            ...
            hid = new BleKeyboard(sasConfigPins.bleName, "SASFW", 100);
        } else {
#if defined(USB_as_HID)
    ...
    if (ble) {
        if (hid->isConnected()) {
            ...
            hid->setLayout(keyboardLayouts[sasConfig.badUSBBLEKeyboardLayout]);
            hid->setDelay(sasConfig.badUSBBLEKeyDelay);
            return;
        }
        if (!_Ask_for_restart) _Ask_for_restart = 1; // arm the flag
        hid->begin(keyboardLayouts[sasConfig.badUSBBLEKeyboardLayout]);
        hid->setDelay(sasConfig.badUSBBLEKeyDelay);
    } else {
#if defined(USB_as_HID)
        hid->begin(keyboardLayouts[sasConfig.badUSBBLEKeyboardLayout]);
        ...
        mySerial.begin(CH9329_DEFAULT_BAUDRATE, SERIAL_8N1, BAD_RX, BAD_TX);
        delay(100);
        hid->begin(mySerial, keyboardLayouts[sasConfig.badUSBBLEKeyboardLayout]);
        hid->setDelay(sasConfig.badUSBBLEKeyDelay);
#endif
    }
}
...
void ducky_setup(HIDInterface *&hid, bool ble) {
    ...
    if (ble && sasConfig.badUSBBLEKeyDelay < 50) {
...
void key_input(FS fs, String bad_script, HIDInterface *_hid) {
    ...
    static int defaultStringDelay = sasConfig.badUSBBLEKeyDelay; // Default delay for all STRING commands
    currentOutputY = 0;
    ...
    if (!sasConfig.badUSBBLEShowOutput) {
        tft.setTextSize(FP);
    ...
    while (payloadFile.available()) {
        ...
        for (i = 0; i < repeatCount; i++) {
            ...
            if (PriCmd == nullptr) {
                ...
                printTFTBadUSBBLE(Command, sasConfig.priColor);
...
void sendAltChar(HIDInterface *hid, uint8_t charCode) {
    // Hold ALT key
    hid->press(KEY_LEFT_ALT);
    delay(sasConfig.badUSBBLEKeyDelay);
...
void printTextAtPosition(uint16_t xOffset, uint16_t yOffset, const String &text) {
    ...
    tft.setCursor(x, y);
    tft.fillRect(x, y, tftWidth - x - BORDER_OFFSET_FROM_SCREEN_EDGE * 2, FP * 8, sasConfig.bgColor);
    tft.print(text);
...
void printTFTBadUSBBLE(String text, uint16_t color, bool newline) {
    if (!sasConfig.badUSBBLEShowOutput) return;
...
void PresenterMode(HIDInterface *&hid, bool ble) {
    ...
    auto drawStaticUI = [&]() {
        ...
        tft.drawFastHLine(10, 35, tftWidth - 20, sasConfig.priColor);
    ...
    auto updateSlideDisplay = [&]() {
        ...
        tft.fillRect(0, tftHeight / 2 - 35, tftWidth, 40, sasConfig.bgColor);

        // Draw current slide number - large and centered
        tft.setTextSize(4);
        tft.setTextColor(TFT_WHITE, sasConfig.bgColor);
    ...
    auto updateTimerDisplay = [&]() {
        ...
        tft.fillRect(0, tftHeight / 2 + 30, tftWidth, 30, sasConfig.bgColor);
        tft.setTextSize(3);
        tft.setTextColor(timerStarted ? TFT_GREEN : TFT_DARKGREY, sasConfig.bgColor);
