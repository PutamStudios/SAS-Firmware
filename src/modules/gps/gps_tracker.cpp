...
bool GPSTracker::begin_gps() {
    releasePins();
    GPSserial.begin(
        sasConfigPins.gpsBaudrate, SERIAL_8N1, sasConfigPins.gps_bus.rx, sasConfigPins.gps_bus.tx
    );
...
void GPSTracker::add_initial_file_data(File file) {
    ...
    file.println("  creator=\"SAS Firmware\"");
    ...
    file.println("    <name>SAS GPS Tracker</name>");
    file.println("    <desc>GPS Tracker using SAS Firmware</desc>");
    file.println("    <link href=\"https://sas.computer\">");
    file.println("      <text>SAS Website</text>");
    file.println("    </link>");
    file.println("  </metadata>");
    file.println("  <trk>");
    file.println("    <name>SAS Route</name>");
    file.println("    <desc>GPS route captured by SAS firmware</desc>");
    file.println("    <trkseg>");
}
...
void GPSTracker::add_final_file_data() {
    ...
    if (filename == "" || !(*fs).exists("/SASGPS/" + filename)) return;

    File file = (*fs).open("/SASGPS/" + filename, FILE_APPEND);
...
void GPSTracker::add_coord() {
    ...
    if (!(*fs).exists("/SASGPS")) (*fs).mkdir("/SASGPS");

    bool is_new_file = false;
    if (!(*fs).exists("/SASGPS/" + filename)) is_new_file = true;
    File file = (*fs).open("/SASGPS/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);
...
void GPSTracker::releasePins() {
    rxPinReleased = false;
    if (sasConfigPins.CC1101_bus.checkConflict(sasConfigPins.gps_bus.rx) ||
        sasConfigPins.NRF24_bus.checkConflict(sasConfigPins.gps_bus.rx) ||
#if !defined(LITE_VERSION)
        sasConfigPins.W5500_bus.checkConflict(sasConfigPins.gps_bus.rx) ||
        sasConfigPins.LoRa_bus.checkConflict(sasConfigPins.gps_bus.rx) ||
#endif
        sasConfigPins.SDCARD_bus.checkConflict(sasConfigPins.gps_bus.rx)) {
        // T-Embed CC1101 and T-Display S3 Touch ties this pin to the NRF24 CS; switch it to input so the GPS
        // UART can drive it.
        pinMode(sasConfigPins.gps_bus.rx, INPUT);
        rxPinReleased = true;
    }
}
...
void GPSTracker::restorePins() {
    if (rxPinReleased) {
        if (sasConfigPins.CC1101_bus.checkConflict(sasConfigPins.gps_bus.rx) ||
            ...
            pinMode(sasConfigPins.gps_bus.rx, OUTPUT);
            if (sasConfigPins.gps_bus.rx == sasConfigPins.CC1101_bus.cs ||
                sasConfigPins.gps_bus.rx == sasConfigPins.NRF24_bus.cs ||
#if !defined(LITE_VERSION)
                sasConfigPins.gps_bus.rx == sasConfigPins.W5500_bus.cs ||
                sasConfigPins.gps_bus.rx == sasConfigPins.W5500_bus.cs ||
#endif
                sasConfigPins.gps_bus.rx == sasConfigPins.SDCARD_bus.cs) {
                // If it is conflicting to an SPI CS pin, keep it HIGH
                digitalWrite(sasConfigPins.gps_bus.rx, HIGH);
            } else {
                // If it is conflicting with any other SPI pin, keep it LOW
                // Avoids CC1101 Jamming and nRF24 radio to keep enabled
                digitalWrite(sasConfigPins.gps_bus.rx, LOW);
            }
        }
        rxPinReleased = false;
    }
