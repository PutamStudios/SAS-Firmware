...
void Wardriving::scanWiFiBLE() {
    ...
    if (!(*fs).exists("/SASWardriving")) (*fs).mkdir("/SASWardriving");

    bool is_new_file = false;
    if (!(*fs).exists("/SASWardriving/" + filename)) is_new_file = true;
    File file = (*fs).open("/SASWardriving/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);
    ...
    if (is_new_file) {
        file.println(
            "WigleWifi-1.6,appRelease=v" + String(SAS_VERSION) + ",model=M5Stack GPS Unit,release=v" +
            String(SAS_VERSION) +
            ",device=ESP32 M5Stack,display=SPI TFT,board=ESP32 M5Stack,brand=SAS,star=Sol,body=4,subBody=1"
        );
        file.println(
...
void Wardriving::loadAlertMACs() {
    ...
    if ((*fs).exists("/SASWardriving/alert.txt")) {
        File alertFile = (*fs).open("/SASWardriving/alert.txt", FILE_READ);
        if (alertFile) {
        ...
        File alertFile = (*fs).open("/SASWardriving/alert.txt", FILE_WRITE);
        if (alertFile) {
