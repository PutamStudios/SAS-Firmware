...
void begin_storage() {
    ...
    sasConfig.fromFile(checkFS);
    sasConfigPins.fromFile(checkFS);
}
...
void setup_gpio() {
...
#endif
        if (sasConfigPins.CC1101_bus.mosi == sasConfigPins.SDCARD_bus.mosi)
    ...
    // other devices (need to change it when SAS board comes to shore)
}
 ...
 *********************************************************************/
void begin_tft() {
    tft.setRotation(sasConfigPins.rotation); // sometimes it misses the first command
    tft.invertDisplay(sasConfig.colorInverted);
    tft.setRotation(sasConfigPins.rotation);
    tftWidth = tft.width();
#ifdef HAS_TOUCH
...
void boot_screen() {
    ...
    tft.drawCentreString("SAS", tftWidth / 2, 10, 1);
    tft.setTextSize(FP);
    tft.drawCentreString(SAS_VERSION, tftWidth / 2, 25, 1);
    tft.setTextSize(FM);
...
void boot_screen_anim() {
    ...
    if (sasConfig.theme.boot_img) boot_img = 5; // override others

    // FORCE our custom cat boot animation from internal flash memory!
    boot_img = 0;

    bool small_cat_drawn = false;
    bool full_cat_drawn = false;

    tft.drawPixel(0, 0, 0);       // Forces back communication with TFT, to avoid ghosting
                                  // Start image loop
    while (millis() < i + 7000) { // boot image lasts for 5 secs
        if ((millis() - i > 2000) && !drawn) {
            tft.fillRect(0, 45, tftWidth, tftHeight - 45, sasConfig.bgColor);
            if (boot_img > 0 && !drawn) {
                tft.fillScreen(sasConfig.bgColor);
                if (boot_img == 5) {
                    drawImg(
                        *sasConfig.themeFS(),
                        sasConfig.getThemeItemImg(sasConfig.theme.paths.boot_img),
                        0,
                        0,
                        true,
                        3600
        ...
        if (!boot_img && (millis() - i > 2200) && (millis() - i) < 2700)
            tft.drawRect(2 * tftWidth / 3, tftHeight / 2, 2, 2, sasConfig.priColor);
        if (!boot_img && (millis() - i > 2700) && (millis() - i) < 2900)
            tft.fillRect(0, 45, tftWidth, tftHeight - 45, sasConfig.bgColor);
        if (!boot_img && (millis() - i > 2900) && (millis() - i) < 3400) {
            if (!small_cat_drawn) {
                int oldRot = tft.getRotation();
                tft.setRotation(3); // Force flipped landscape
                ...
                tft.setRotation(oldRot); // Restore portrait
                small_cat_drawn = true;
            }
        }
        if (!boot_img && (millis() - i > 3400) && (millis() - i) < 3600) tft.fillScreen(sasConfig.bgColor);
        if (!boot_img && (millis() - i > 3600)) {
            if (!full_cat_drawn) {
                int oldRot = tft.getRotation();
                tft.setRotation(3); // Force flipped landscape for 240x135 image
                bool oldSwapBytes = tft.getSwapBytes();
                tft.setSwapBytes(true);
                tft.pushImage(
                    (240 - 240) / 2,
 ...
 *********************************************************************/
void startup_sound() {
    if (sasConfig.soundEnabled == 0) return; // if sound is disabled, do not play sound
#if !defined(LITE_VERSION)
#if defined(BUZZ_PIN)
    ...
    // play a boot sound
    if (sasConfig.theme.boot_sound) {
        playAudioFile(sasConfig.themeFS(), sasConfig.getThemeItemImg(sasConfig.theme.paths.boot_sound));
...
void setup() {
    ...
    wifiConnected = false;
    BLEConnected = false;
    sasConfig.bright = 100; // theres is no value yet
    sasConfigPins.rotation = ROTATION;
    setup_gpio();
#if defined(HAS_SCREEN)
    tft.init();
    tft.setRotation(sasConfigPins.rotation);
    tft.fillScreen(TFT_BLACK);
    // sasConfig is not read yet.. just to show something on screen due to long boot time
    tft.setTextColor(TFT_PURPLE, TFT_BLACK);
    tft.drawCentreString("SAS BOOTING...", tft.width() / 2, tft.height() / 2, 1);
#else
    tft.begin();
#endif
    begin_storage();
    ...
    // #endif
#if defined(HAS_SCREEN)
    sasConfig.openThemeFile(sasConfig.themeFS(), sasConfig.themePath, false);
    if (!sasConfig.instantBoot) {
        boot_screen_anim();
        startup_sound();
    }
    if (sasConfig.wifiAtStartup) {
    ...
    wakeUpScreen();
    if (sasConfig.startupApp != "" && !startupApp.startApp(sasConfig.startupApp)) {
        sasConfig.setStartupApp("");
    }
}
...
void loop() {
    ...
    Serial.println(
        ...
        "      Type 'sas' to see device diagnostics\n\n"
        "At your command:"
    );
