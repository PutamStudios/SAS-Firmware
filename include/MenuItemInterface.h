...
public:
    ...
    virtual void drawIconImg() {
        drawImg(
            *sasConfig.themeFS(),
            sasConfig.getThemeItemImg(themePath()),
            0,
            imgCenterY,
            true,
            sasConfig.theme.gifDuration,
            false
        );
    }
    ...
    void draw(float scale = 1) {
        if (rotation != sasConfigPins.rotation) resetCoordinates();
        if (!checkTheme()) {
            tft.fillRect(0, 27, tftWidth, tftHeight - 27, sasConfig.bgColor);
            drawIcon(scale);
            drawArrows(scale);
            drawTitle(scale);
        } else {
            if (sasConfig.theme.label)
                drawTitle(scale); // If using .GIF, labels are draw after complete, which takes some time
            drawIconImg();
            if (sasConfig.theme.label) drawTitle(scale); // Makes sure to draw over the image
        }
        drawStatusBar();
    }

    void drawArrows(float scale = 1) {
        tft.fillRect(arrowAreaX, iconAreaY, arrowAreaW, iconAreaH, sasConfig.bgColor);
        tft.fillRect(
            tftWidth - arrowAreaX - arrowAreaW, iconAreaY, arrowAreaW, iconAreaH, sasConfig.bgColor
        );
    ...
    void drawTitle(float scale = 1) {
        ...
        tft.setTextSize(FM);
        tft.drawPixel(0, 0, 0);
        tft.fillRect(arrowAreaX, titleY, tftWidth - 2 * arrowAreaX, LH * FM, sasConfig.bgColor);
...
public:
    ...
    void setCoordinates(int centerX, int centerY, int areaW, int areaH, int areaX, int areaY) {
        iconCenterX = centerX;
        iconCenterY = centerY;
        iconAreaW = areaW;
        iconAreaH = areaH;
    ...
    void resetCoordinates(void) {
        ...
        rotation = sasConfigPins.rotation;
    }
...
protected:
    ...
    void clearIconArea(void) {
        tft.fillRect(iconAreaX, iconAreaY, iconAreaW, iconAreaH, sasConfig.bgColor);
    }
    void clearImgArea(void) { tft.fillRect(7, 27, tftWidth - 14, tftHeight - 34, sasConfig.bgColor); }
