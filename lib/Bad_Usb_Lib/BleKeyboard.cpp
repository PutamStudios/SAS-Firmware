...
void BleKeyboard::CharacteristicCallbacks::onSubscribe(
    ...
    if (subValue == 0) {
        Serial.println("SAS KEYBOARD: Client unsubscribed from notifications/indications.");
        if (parent->m_subCount) parent->m_subCount--;
    } else {
        parent->m_subCount++;
        Serial.println("SAS KEYBOARD: Client subscribed to notifications.");
    }
}
